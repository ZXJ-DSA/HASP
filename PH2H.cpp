...
void Graph::IndexConstruction() {
    ...
    switch (systemIndex) {
        ...
        case HTSPIndex::MVCC_PostMHL: {
            cout << "System Index: MVCC_PostMHL index." << endl;
            cout << "Bandwidth: " << bandWidth << endl;
            PostMHLIndexConstruct();
            break;
        }
        ...
        case HTSPIndex::DVPLwoRV: {
            cout << "System Index: DVPLwoRV index." << endl;
            string orderFile = sourcePath + "/partitions/" + dataset + "_" + algoParti + "_" + to_string(partiNum) +
                               "/vertex_orderMDEGreedy";
            VPLIndexConstruct(orderFile);
            break;
        }
        case HTSPIndex::DVPLwoRVO: {
            cout << "System Index: DVPLwoRVO index." << endl;
            string orderFile = sourcePath + "/partitions/" + dataset + "_" + algoParti + "_" + to_string(partiNum) +
                               "/vertex_orderMDE2";
            VPLIndexConstruct(orderFile);
            break;
...
//    if(true){
    if (!IF.is_open()) {/// if no order file, use MDE to generate order
        ...
        for (int i = 0; i < num; i++) {
            ...
            boost::split(vs, line, boost::is_any_of(" \t"), boost::token_compress_on);
            if (vs.size() != 2) {
                cout << "Wrong syntax for ordering. " << line << endl;
                exit(1);
            }
            nodeID = stoi(vs[0]);
            nodeorder = stoi(vs[1]);
            ...
            if (nodeorder != -1) {
                ...
                cout << "Wrong order! " << nodeID << " " << nodeorder << endl;
                exit(1);
            }
        }
        IF.close();
        ...
        if (vertices.size() != node_num) {
            cout << "Order wrong! " << vertices.size() << " " << node_num << endl;
            exit(1);
        }
        vertices.clear();
        ...
        SCconNodesMT.assign(node_num,
                            map<int, vector<pair<int, int>>>());//record the supportive vertices of a shortcut, only record edge once by leveraging the ID positions of endpoints
...
//                if(true){
                    ...
                    if (Neigh.size() > threadnum) {
                        ...
                        for (int i = 0; i < threadnum; i++) {
                            ...
                            if (i == threadnum - 1)
                                ...
                                p.second = (i + 1) * step;
                            thread.add_thread(
                                    new boost::thread(&Graph::NeighborComorderH2H, this, boost::ref(Neigh), p, x));
                        }
                        thread.join_all();
                    } else {
                        boost::thread_group thread;
                ...
                cout << "Wrong order! " << x << " " << nodeorder << endl;
                exit(1);
            }
        }
    }
...
void Graph::VPLIndexConstruct(string& orderFile) {
    ...
    if (!inFile) { // if not exist
        ...
        inFile.close();
        if (algoChoice == Index_DVPL || algoChoice == static_cast<int>(HTSPIndex::DVPLwoRV) ||
            algoChoice == static_cast<int>(HTSPIndex::DVPLwoR)) {
            PH2HVertexOrdering(3); // Tree height-aware PSP ordering
        } else if (algoChoice == static_cast<int>(HTSPIndex::DVPLwoRVO) || algoChoice == Index_PMHL ||
...
void Graph::PMHLIndexConstructOpt() {
    ...
    string orderfile;
    orderfile =
            sourcePath + "/partitions/" + dataset + "_" + algoParti + "_" + to_string(partiNum) + "/vertex_orderMDE2";
    ReadOrder(orderfile);
...
//            PartitionOrderingBoundaryFirstMDE(false, change);
        ...
        default: {
            cout << "Wrong ordering type! " << type << endl;
            exit(1);
        }
    }
    tt.stop();
...
void Graph::OrderingAssemblyMDEBoundaryFirstGreedy(int pNum) {
    string filename =
            sourcePath + "/partitions/" + dataset + "_" + algoParti + "_" + to_string(pNum) + "/vertex_orderMDEGreedy";
...
//    cout<<verticesV.size()<<" "<<vertices.size()<<endl;
    if (order_i != node_num || vertices.size() != node_num || vNodeOrder.size() != node_num) {
        cout << "Wrong order number! " << order_i << " " << vertices.size() << " " << vNodeOrder.size() << " "
             << node_num << endl;
        exit(1);
    }
    ...
    for (int i = 0; i < nodeNumOverlay; ++i) {
        ...
        if (!PartiTag[ID].second) {
            cout << nodeNumOverlay << ", " << node_num - i << ": " << ID << "(" << NodeOrder[ID]
                 << ") is not boundary vertex!" << endl;
            exit(1);
        }
    }
    ...
    for (int i = 0; i < NodeOrder.size(); i++) {
        if (NodeOrder[i] == -1) {
            cout << "Wrong order! " << i << "(" << PartiTag[i].first << ") " << NodeOrder[i] << endl;
            exit(1);
        }
...
void Graph::OrderingAssemblyMDEBoundaryFirst(int pNum) {
    string filename =
            sourcePath + "/partitions/" + dataset + "_" + algoParti + "_" + to_string(pNum) + "/vertex_orderMDE2";
...
void Graph::OrderingAssemblyMDE(int pNum) {
    string filename =
            sourcePath + "/partitions/" + dataset + "_" + algoParti + "_" + to_string(pNum) + "/vertex_orderMDE";
...
//    cout<<verticesV.size()<<" "<<vertices.size()<<endl;
    if (order_i != node_num) {
        cout << "Wrong order number! " << order_i << " " << node_num << endl;
        exit(1);
    }
    OF << node_num << endl;
    for (int i = 0; i < NodeOrder.size(); i++) {
...
void Graph::OrderingAssemblyBoundaryFirst(int pNum) {
    string orderfile =
            sourcePath + "/partitions/" + dataset + "_" + algoParti + "_" + to_string(pNum) + "/vertex_order2";
...
//            }
    ...
    if (nNum != node_num) {
        cout << "Inconsistent node number! " << nNum << " " << node_num << endl;
        exit(1);
    }
...
//            }
            if (ID1 >= 0 && ID1 < node_num && ID2 >= 0 && ID2 < node_num && weight > 0) {
                ...
                cout << "Wrong for subgraph_edge! " << ID1 << " " << ID2 << " " << weight << endl;
                exit(1);
            }
...
//            NeighborsOverlay[ID1].emplace_back(ID2,weight);
            ...
            if (NeighborSketchS[PID1].find(PID2) ==
                NeighborSketchS[PID1].end()) {//if not found, i.e., PID2 is not in the NeighborSketchS of PID1
            ...
            cout << "Wrong for cut_edge! " << ID1 << " " << ID2 << " " << weight << endl;
            exit(1);
        }
    }
...
void Graph::OverlayOrderingBuild() {
    ...
    for (ID = 0; ID < partiNum; ID++) {
        ...
        if (degree != 0) {
            ...
            cout << "Wrong! degree is zero. " << ID << " " << degree << endl;
            exit(1);
        }
    }

    vector<bool> exist;
    exist.assign(partiNum, true);
    vector<bool> change;
    change.assign(partiNum, false);
    ...
    while (!Deg.empty()) {
        ...
        if (lastParti != -1 &&
            NeighborCon[lastParti].find(x) != NeighborCon[lastParti].end()) {//if x is lastParti's neighbor
            neix.clear();
            ...
            if (NeighborCon[lastParti].find(x) !=
                NeighborCon[lastParti].end()) {//if found, i.e., x is the neighbor of lastParti
...
//        cout<<x<<" "<<Deg.size()<<endl;
    ...
    if (vNodeOrderOverlay.size() != partiNum || pSet.size() != partiNum) {
        cout << "Inconsistent size for sketch graph! " << vNodeOrderOverlay.size() << " " << pSet.size() << " "
             << partiNum << endl;
        exit(1);
    }
...
//    vNodeOrderParti.assign(partiNum,map<int,int>());
    ...
    if (ifParallel) {
        if (threadnum < partiNum) {
            ...
            for (auto j = 0; j < processID.size(); ++j) {
                thread.add_thread(
                        new boost::thread(&Graph::PartitionOrderingBoundaryFirstV, this, boost::ref(processID[j]),
                                          boost::ref(change)));
            }
            thread.join_all();
        } else {//thread number is sufficient
            ...
            for (auto j = 0; j < partiNum; ++j) {
                thread.add_thread(
                        new boost::thread(&Graph::PartitionOrderingBoundaryFirst, this, j, boost::ref(change)));
            }
            thread.join_all();
        }
    } else {
    ...
    cout << "Maximal in-partition tree height: " << maxHeight << " ; average in-partition tree height: "
         << aveHeight / partiNum << " ; boundary-clique partition number: " << cliqueBoundaryPNum << endl;
}
...
//        vNodeOrderParti[pid].insert({order_i,x});
    ...
    if (vNodeOrderParti[pid].size() != PartiVertex[pid].size()) {
        cout << "Inconsistent size! " << pid << " " << vNodeOrderParti[pid].size() << " " << PartiVertex[pid].size()
             << endl;
        exit(1);
    }
}
...
//        }
        ...
        if (algoChoice == Index_MHL) {
            ...
            d2 = QueryPostMHL(s, t);
            tt.stop();
        } else if (algoChoice == Index_DVPL || algoChoice == static_cast<int>(HTSPIndex::DVPLwoRVO) ||
            algoChoice == static_cast<int>(HTSPIndex::DVPLwoR)) {
            tt.start();
...
//            d2=QueryVPL_HASPDebug(s,t);
        ...
        } else if (algoChoice == static_cast<int>(HTSPIndex::DVPLwoRV)) {
            tt.start();
            d2=QueryVPL(s,t);
            tt.stop();
            if (PartiTag[s].first == PartiTag[t].first) {
...
//        cout<<i<<": "<<s<<" "<<t<<" "<<d2<<" "<<d1<<endl;
        if (d1 != d2) {
            if (algoChoice == 1) {
                cout << "Correct Test. InCorrect! Algorithm " << algoQuery << ", " << i << ": " << s << " " << t << ": "
                     << d2 << " " << d1 << endl;
            } else if (algoChoice == 2 || algoChoice == 3 || algoChoice == 4) {
                cout << "Correct Test. InCorrect! Algorithm " << algoQuery << ", " << i << ": " << s << "("
                     << NodeOrder[s] << ") " << t << "(" << NodeOrder[t] << ") " << d2 << " " << d1
                     << " ; Partition Tag: " << PartiTag[s].first << " " << PartiTag[t].first << "; Boundary Tag: "
                     << PartiTag[s].second << " " << PartiTag[t].second << endl;
                QueryDebug(s, t);
            } else if (algoChoice == 5) {
                cout << "Correct Test. InCorrect! Algorithm " << algoQuery << ", " << i << ": " << s << "("
                     << NodeOrder[s] << ") " << t << "(" << NodeOrder[t] << ") " << d2 << " " << d1
                     << " ; Partition Tag: " << PartiTags[s].first << " " << PartiTags[t].first << endl;
...
//                QueryVPLDebug(s,t);
    ...
    if (algoChoice == 6) {
        cout << "Inner-unaffected query number: " << innerBenefitNum << " ; Outer-unaffected query number: "
             << outerBenefitNum << endl;
    }
...
//                        cout<<b1<<" "<<b2<<" "<<d<<endl;
            ...
            dis = d;
            int d_12 = QueryCore(b1, b2), dDijk_s = Dijkstra(ID1, b1, Neighbor), dDijk_12 = Dijkstra(b1, b2,
                                                                                                     Neighbor), dDijk_t = Dijkstra(
                    b2, ID2, Neighbor);
            cout << ID1 << " " << b1 << "(" << NodeOrder[b1] << "," << PartiTag[b1].first << ") " << b2 << "("
                 << NodeOrder[b2] << "," << PartiTag[b2].first << ") " << ID2 << " : " << d1 << " " << d_12 << " " << d2
                 << " ; " << dDijk_s << " " << dDijk_12 << "(" << DijkstraCore(b1, b2) << ") " << dDijk_t << endl;
...
//                dis= QuerySameParti(ID1,ID2);
            ...
            if (b1 != -1) {
                cout << "d4: " << ID1 << " " << b1 << " " << b2 << " " << ID2 << " : " << df1 << " "
                     << QueryCore(b1, b2) << " " << df2 << " ; " << Dijkstra(ID1, b1, Neighbor) << " "
                     << Dijkstra(b1, b2, Neighbor) << " " << Dijkstra(b2, ID2, Neighbor) << endl;
            } else {
...
//            d1=QueryCore(s,t);
            ...
            if (d1 != d2) {
                cout << "InCorrect! " << s << "(" << NodeOrder[s] << ") " << t << "(" << NodeOrder[t] << "): " << d1
                     << " " << d2 << endl;
//				DijkstraPath(s,t);
...
//        }
        if (algoChoice == Index_MHL) {
            ...
            tt.stop();
        } else if (algoChoice == Index_DVPL || algoChoice == static_cast<int>(HTSPIndex::DVPLwoRV) ||
            algoChoice == static_cast<int>(HTSPIndex::DVPLwoRVO) || algoChoice == static_cast<int>(HTSPIndex::DVPLwoR)) {
            tt.start();
            d1 = QueryVPL(ID1, ID2);
            tt.stop();
            if (PartiTag[ID1].first == PartiTag[ID2].first) {
...
//            d1=Dijkstra(ID1,ID2,Neighbor);
        ...
        if (ifDebug) {
            ...
            if (d1 != d2) {
                if (algoChoice == 5) {
                    cout << "Incorrect! " << i << ": " << ID1 << "(" << PartiTags[ID1].first << ") " << ID2 << "("
                         << PartiTags[ID2].first << "): " << d1 << " " << d2 << endl;
                    QueryPostMHLDebug(ID1, ID2);
                    exit(1);
                } else if (algoChoice >= 1 && algoChoice <= 4) {
                    cout << "Incorrect! " << i << ": " << ID1 << "(" << PartiTag[ID1].first << ","
                         << PartiTag[ID1].second << ") " << ID2 << "(" << PartiTag[ID2].first << ","
                         << PartiTag[ID2].second << "): " << d1 << " " << d2 << endl;
                    exit(1);
                }
    ...
    if (algoChoice == Index_PMHLNaive || algoChoice == Index_PMHL || algoChoice == Index_DVPL ||
        algoChoice == static_cast<int>(HTSPIndex::DVPLwoRVO) || algoChoice == static_cast<int>(HTSPIndex::DVPLwoRV) ||
        algoChoice == static_cast<int>(HTSPIndex::DVPLwoR)) {
        cout << "Same-partition query number: " << sameNum << " ; Cross-partition query number: " << crossNum << endl;
    }
...
//function for efficiency test
void
Graph::EffiCheckStages(vector<pair<int, int>> &ODpair, int runtimes, int intervalT, unsigned long long &throughputNum,
                       vector<double> &stageUpdateT, vector<double> &stageQueryT) {
    cout << "Efficiency test. Run times: " << runtimes << endl;
    int s, t;
    Timer tt;
...
//    ifDebug=true;
    ...
    if (algoChoice == Index_MHL) {
        ...
        if (updateT < intervalT) {
            dt1 = stageDurations[Dijk];
            qt1 = dt1 / runT1;
            //Stage 2: CH
            updateT += stageDurations[CH];
            if (updateT < intervalT) {
                dt2 = stageDurations[CH];
                qt2 = dt2 / runT2;
                //Stage 3: CH
                stageDurations[H2H] = intervalT - updateT;
                dt3 = stageDurations[H2H];
                qt3 = dt3 / runT3;
            } else {
                dt2 = intervalT - stageDurations[Dijk];
                qt2 = dt2 / runT2;
            }
        } else {
            dt1 = intervalT;
        ...
        qtAll = qt1 + qt2 + qt3;
        cout << "Stage 1 (BiDijkstra): Duration: " << dt1 << " (" << stageDurations[Dijk] << ") s; Throughput number: "
             << qt1 << " ; query time: " << 1000 * runT1 << " ms." << endl;
        cout << "Stage 2 (CH): Duration: " << dt2 << " (" << stageDurations[CH] << ") s; Throughput number: " << qt2
             << " ; query time: " << 1000 * runT2 << " ms." << endl;
        cout << "Stage 3 (H2H): Duration: " << dt3 << " (" << stageDurations[H2H] << ") s; Throughput number: " << qt3
             << " ; query time: " << 1000 * runT3 << " ms." << endl;
        ...
        if (updateT < intervalT) {
            ...
            if (updateT < intervalT) {
                dt2 = stageDurations[PCH_No];
                qt2 = dt2 / runT2;
                //Stage 3: No-boundary PMHL
                updateT += stageDurations[PH2H_No];
                if (updateT < intervalT) {
                    dt3 = stageDurations[PH2H_No];
                    qt3 = dt3 / runT3;
                    //Stage 4: Post-boundary PMHL
                    updateT += stageDurations[PH2H_Post];
                    if (updateT < intervalT) {
                        dt4 = stageDurations[PH2H_Post];
                        qt4 = dt4 / runT4;
                        //Stage 5: Cross-boundary PMHL
                        stageDurations[PH2H_Cross] = intervalT - updateT;
                        dt5 = stageDurations[PH2H_Cross];
                        qt5 = dt5 / runT5;
                    } else {
                        dt4 = intervalT - stageDurations[Dijk] - stageDurations[PCH_No] - stageDurations[PH2H_No];
                        qt4 = dt4 / runT4;
                    }
                } else {
                    dt3 = intervalT - stageDurations[Dijk] - stageDurations[PCH_No];
                    qt3 = dt3 / runT3;
                }
            } else {
                dt2 = intervalT - stageDurations[Dijk];
        ...
        cout << "Stage 1 (Dijkstra): Duration: " << dt1 << " (" << stageDurations[Dijk] << ") s; Throughput number: "
             << qt1 << " ; query time: " << 1000 * runT1 << " ms." << endl;
        cout << "Stage 2 (PCH): Duration: " << dt2 << " (" << stageDurations[PCH_No] << ") s; Throughput number: "
             << qt2 << " ; query time: " << 1000 * runT2 << " ms." << endl;
        cout << "Stage 3 (PH2H-No): Duration: " << dt3 << " (" << stageDurations[PH2H_No] << ") s; Throughput number: "
             << qt3 << " ; query time: " << 1000 * runT3 << " ms." << endl;
        cout << "Stage 4 (PH2H-Post): Duration: " << dt4 << " (" << stageDurations[PH2H_Post]
             << ") s; Throughput number: " << qt4 << " ; query time: " << 1000 * runT4 << " ms." << endl;
        cout << "Stage 5 (PH2H-Extend): Duration: " << dt5 << " (" << stageDurations[PH2H_Cross]
             << ") s; Throughput number: " << qt5 << " ; query time: " << 1000 * runT5 << " ms." << endl;
        cout << "Throughput number: " << qtAll << endl;
        throughputNum = throughputNum + qtAll;
        stageUpdateT[Dijk] += stageDurations[Dijk];
        ...
        if (updateT < intervalT) {
            ...
            if (updateT < intervalT) {
                ...
                if (updateT < intervalT) {
                    ...
                    dt4 = intervalT - stageDurations[Dijk] - stageDurations[PCH_No];
                    qt4 = dt4 / runT4;
                }
            } else {
                dt2 = intervalT - stageDurations[Dijk];
        ...
        cout << "Stage 2 (PCH): Duration: " << dt2 << " (" << stageDurations[PCH_No] << ") s; Throughput number: "
             << qt2 << " ; query time: " << 1000 * runT2 << " ms." << endl;
        cout << "Stage 3 (PH2H-Post): Duration: " << dt4 << " (" << stageDurations[PH2H_Post]
             << ") s; Throughput number: " << qt4 << " ; query time: " << 1000 * runT4 << " ms." << endl;
        cout << "Stage 4 (PH2H-Extend): Duration: " << dt5 << " (" << stageDurations[PH2H_Cross]
             << ") s; Throughput number: " << qt5 << " ; query time: " << 1000 * runT5 << " ms." << endl;
        cout << "Throughput number: " << qtAll << endl;
        throughputNum = throughputNum + qtAll;
        stageUpdateT[Dijk] += stageDurations[Dijk];
        ...
        stageQueryT[PH2H_Cross] += runT5;
               ...
               algoChoice == static_cast<int>(HTSPIndex::DVPLwoRV) || algoChoice == static_cast<int>(HTSPIndex::DVPLwoR)) {
        ...
        cout << "Stage 1 (Dijkstra): Duration: " << dt1 << " (" << stageDurations[Dijk] << ") s; Throughput number: "
             << qt1 << " ; query time: " << 1000 * runT1 << " ms." << endl;
        cout << "Stage 2 (MPHL-CH): Duration: " << dt2 << " (" << stageDurations[PCH_No] << ") s; Throughput number: "
             << qt2 << " ; query time: " << 1000 * runT2 << " ms." << endl;
        cout << "Stage 3 (MPHL-Post): Duration: " << dt4 << " (" << stageDurations[PH2H_Post]
             << ") s; Throughput number: " << qt4 << " ; query time: " << 1000 * runT4 << " ms." << endl;
        cout << "Stage 4 (MPHL-Extend): Duration: " << dt5 << " (" << stageDurations[PH2H_Cross]
             << ") s; Throughput number: " << qt5 << " ; query time: " << 1000 * runT5 << " ms." << endl;
        cout << "Throughput number: " << qtAll << endl;
        throughputNum = throughputNum + qtAll;
        stageUpdateT[Dijk] += stageDurations[Dijk];
            ...
            if (updateT < intervalT) {
                ...
                if (updateT < intervalT) {
                    ...
                    if (updateT < intervalT) {
                        ...
                        durations[PH2H_Post] =
                                intervalT - stageDurations[Dijk] - stageDurations[PCH_No] - stageDurations[PH2H_No];
                    }
                } else {
...
//function for efficiency test
void Graph::EffiStageCheck(vector<pair<int, int>> &ODpair, int runtimes,
                           vector<vector<double>> &queryTimes) {//return in seconds
    cout << "Efficiency test. Run times: " << runtimes << endl;
    int s, t;
    Timer tt;
    tt.start();
...
//function for efficiency test
void Graph::EffiStageCheck(vector<pair<int, int>> &ODpair, int runtimes, vector<vector<double>> &queryTimes,
                           vector<double> &aveQT) {//return in seconds
...
//function for efficiency test
void Graph::EffiStageCheckReal(vector<pair<int, int>> &ODpair, int runtimes,
                               vector<vector<double>> &queryTimes) {//return in seconds
...
double Graph::analytical_update_first(double T_q, double T_u, double T_r, double T,
                                      double V_q) {//T is duration, T_u is the average update time for one update, return throughput
...
pair<double, double>
Graph::ThroughputEstimate(vector<vector<double>> &query_costs, vector<vector<double>> &update_costs,
                          double threshold_time, double T) {
    double queryT, duration, queryT_var;
...
//        cout<<i<<" . update time: "<<update_time<<" s ; estimate duration: "<<durationT<<" s"<<endl;
        ...
        cout << "Throughput of Q-stage " << i + 1 << " : " << thr << " ; duration: " << duration << " s ; query time: "
             << queryT * 1000 << " ms" << endl;
        throughput += thr * tau;
    }
...
//update first model
pair<double, double>
Graph::simulator_UpdateFirst(vector<vector<double>> &query_cost, vector<vector<double>> &update_cost,
                             vector<query> &queryList, int T, double period_time) {
...
//        cout<<"batch "<<current_update_index<<endl;
        ...
        c_time = period_time *
                 current_update_index;  // + batch_update_costs[current_update_index%batch_update_costs.size()];
...
//            cout<<"query "<<count<<": "<<current_query->init_time<<" "<<current_query->process_time<<" ";
                current_query->process_time = query_cost[i][query_processed_in_one_update_slot %
                                                            query_cost[i].size()];//obtain the true query processing time
//            cout<<current_query->process_time<<endl;
                ...
                if (c_time + current_query->process_time <= durationPoint) {
                    ...
                    avg_response_time += (c_time - current_query->init_time +
                                          current_query->process_time);//obtain the query response time
...
//        }
    ...
    return make_pair(count * 1.0 / T,
                     avg_response_time / count);//return throughput (query per second) and average response time (in us)
}
...
//                    cout<<"!!! Finished the processing of all queries."<<endl;
                    ...
                    if (c_times[topID] + current_query->process_time <= durationPoint) {
                        count++;
                        query_processed_in_one_update_slot++;
                        avg_response_time += (c_times[topID] - current_query->init_time +
                                              current_query->process_time);//obtain the query response time
//                simulated_query_costs.push_back(current_query->process_time);
...
//update first model, multiple workers
pair<double, double>
Graph::simulator_UpdateFirstMVCC(vector<vector<double>> &query_cost, vector<vector<double>> &update_cost,
                             vector<query> &queryList, int T, double period_time, int workerNum) {
    int total_time; // terminal time for each period
    unsigned long long int count = 0; // the number of queries processed.
...
//                    cout<<"!!! Finished the processing of all queries."<<endl;
                    ...
                    current_query->process_time = query_cost[posLast][query_processed_in_one_update_slot %
                                                                query_cost[posLast].size()];//obtain the true query processing time
                    if (current_query->init_time > c_times[topID]) {
                        c_times[topID] = current_query->init_time;
                    }
                    //c_time = max(c_time, current_query->init_time*1000000.0);
...
//    cout << ID1 << "(" << pid1 << ") " << pid2 << "(" << pid2 << ") " << endl;
//    cout << "Q stage: " << Qstage_i << endl;
//    cout << "V-Stage 1: " << partiAffect[pid1].first << " ; V-Stage 2: pid1 " << partiAffect[pid1].second << " , pid2 " << partiAffect[pid2].second << endl;
...
//update first model, multiple workers
pair<double, double> Graph::simulator_UpdateFirstVPL(vector<pair<int, int>> &ODpair, vector<vector<double>> &query_cost,
                                                     vector<vector<double>> &update_cost, vector<query> &queryList,
                                                     int T, double period_time, int workerNum) {
    int total_time; // terminal time for each period
    unsigned long long int count = 0; // the number of queries processed.
...
//            cout<<"query "<<count<<": "<<current_query->init_time<<" "<<current_query->process_time<<" ";
                    ...
                    if (i < PH2H_Cross) {
                        ...
                        ID1 = ODpair[query_j].first, ID2 = ODpair[query_j].second; // query in processing
...
double Graph::ThroughputSimulate(vector<vector<double>> &query_costs, vector<vector<double>> &update_costs,
                                 int simulation_time, double threshold_time, double period_time, double queryTime,
                                 int workerNum) {//period_time is update interval time, simulation_time is the overall time for simulation
    Timer tt;
    tt.start();
    ...
    cout << "Simulate the throughput of system... r: " << r << " ; gap: " << gap << " ; worker number: " << workerNum
         << endl;
...
//        pair<double,double> result = simulator_UpdateFirst(query_costs, update_costs, queryList, simulation_time, period_time);
        pair<double, double> result = simulator_UpdateFirst(query_costs, update_costs, queryList, simulation_time,
                                                            period_time, workerNum);
...
//                cout<<"here:" << result.first<<endl;
                ...
                cout << " ; throughput: " << result.first << " ; response query time: " << result.second * 1000
                     << " ms";
            }
            l = lambda + gap;
//            l=l*1.1;
...
// throughput simulation for index without index validation
double Graph::ThroughputSimulateReal(vector<vector<double>> &query_costs, vector<vector<double>> &update_costs,
                                     int simulation_time, double threshold_time, double period_time, double queryTime,
                                     int workerNum) {//period_time is update interval time, simulation_time is the overall time for simulation
    Timer tt;
    tt.start();
...
// Throughput simulation for MVCC algorithm
double Graph::ThroughputSimulateRealMVCC(vector<vector<double>> &query_costs, vector<vector<double>> &update_costs,
                                     int simulation_time, double threshold_time, double period_time, double queryTime,
                                     int workerNum) {//period_time is update interval time, simulation_time is the overall time for simulation
    Timer tt;
    tt.start();
...
//        pair<double,double> result = simulator_UpdateFirst(query_costs, update_costs, queryList, simulation_time, period_time);
        pair<double, double> result = simulator_UpdateFirstMVCC(query_costs, update_costs, queryList, simulation_time,
                                                            period_time, workerNum);
...
// function for simulating the system throughput
double Graph::ThroughputSimulateRealVPL(vector<pair<int, int>> &ODpair, vector<vector<double>> &query_costs,
                                        vector<vector<double>> &update_costs, int simulation_time,
                                        double threshold_time, double period_time, double queryTime,
                                        int workerNum) {//period_time is update interval time, simulation_time is the overall time for simulation
    Timer tt;
    tt.start();
    double l = 0, r = 2000000; // be careful of 'r'
    ...
    while (l < r) {//why use different lambda?
        ...
        cout << "lambda: " << lambda << " ; the size of querylist: " << queryList.size();
        innerBenefitNum = 0, outerBenefitNum = 0;
        queryNum = 0;
//        pair<double,double> result = simulator_UpdateFirst(query_costs, update_costs, queryList, simulation_time, period_time);
        pair<double, double> result = simulator_UpdateFirstVPL(ODpair, query_costs, update_costs, queryList,
                                                               simulation_time, period_time, workerNum);
...
//                cout<<"lambda: "<<lambda<<" ; the size of querylist: "<<queryList.size();
                cout << " ; throughput: " << result.first << " ; response query time: " << result.second * 1000
                     << " ms ; unaffected query number: " << innerBenefitNum + outerBenefitNum << " ( " << queryNum
                     << " )";
            }
//            gap=(r-l)/10;
//            if(gap<5) gap=1;
...
//    ifDebug=true;
    ...
    for (int i = 0; i < runtimes; ++i) {
        ...
        if (ifDebug) {
            d2 = Dijkstra(ID1, ID2, Neighbor);
            if (d1 != d2) {
                cout << "Wrong result! " << ID1 << " " << ID2 << " " << d1 << " " << d2 << endl;
                exit(1);
            }
        }
    }
...
//    ifDebug=true;
    ...
    for (int i = 0; i < runtimes; ++i) {
        ...
        if (ifDebug) {
            d2 = Dijkstra(ID1, ID2, Neighbor);
            if (d1 != d2) {
                cout << "Wrong result! " << ID1 << "(" << PartiTag[ID1].first << "," << PartiTag[ID1].second << ") "
                     << ID2 << "(" << PartiTag[ID2].first << "," << PartiTag[ID2].second << ") " << d1 << " " << d2
                     << endl;
                exit(1);
            }
        }
...
void Graph::RealUpdateThroughputTest(string updateFile) {
    ...
    cout << "Update batch number: " << batchNum << " ; Average batch size: " << aveBatchSize
         << " ; Maximal batch size: " << maxBatchSize << " ; Minimal batch size: " << minBatchSize
         << " ; Batch interval: " << batchInterval << endl;

    string queryF = sourcePath + "/" + dataset + ".query";
    if (samePartiPortion != -1) {
        queryF = sourcePath + "/tmp/" + dataset + ".PostMHL_" + to_string(bandWidth) + ".sameParti_" +
                 to_string(samePartiPortion) + ".query";
    }
    ifstream IF2(queryF);
    if (!IF2) {
...
//                        cout<<"Inc "<<ID1<<" "<<ID2<<" "<<oldW<<" "<<weight<<endl;
            ...
            if (uEdges.find(make_pair(ID1, ID2)) == uEdges.end()) {//if not found
                ...
                cout << "Wrong. Find. " << ID1 << " " << ID2 << " " << weight << " " << uEdges[make_pair(ID1, ID2)]
                     << " " << oldW << endl;
                exit(1);
            }

            if (!ifFind) {
                cout << "Wrong edge update. " << ID1 << " " << ID2 << " " << endl;
                exit(1);
            }
        }
        cout << "Batch " << i << " . Decrease update number: " << wBatchDec.size() << " ; Increase update number: "
             << wBatchInc.size() << endl;
...
//    EffiCheckStages(ODpair,runtimes,batchInterval,throughputNum,stageUpdateT,stageQueryT);
    ...
    cout << "\nPartiNum: " << partiNum << ". Overall throughput: " << throughputNum << " ; Average throughput: "
         << throughputNum / batchNum << " ; Average batch update Time: " << updateTime / batchNum << " s." << endl;
}
...
void Graph::PostMHLIndexStoreCH(string filename) {
    ...
    for (int i = 0; i < Tree.size(); ++i) {
        ...
        for (int j = 0; j != Tree[i].vert.size(); ++j) {
            OF << " " << Tree[i].vert[j].first << " " << Tree[i].vert[j].second.first << " "
               << Tree[i].vert[j].second.second;
        }
        OF << endl;
    }
    OF.close();
...
//    }
    ...
    int vNum = stoi(vs[0]);
    if (vNum != node_num) {
        cout << "Wrong node number. " << vNum << " " << node_num << endl;
        exit(1);
    }
        ...
        for (auto it = Tree[i].vert.begin(); it != Tree[i].vert.end(); ++it) {
            ...
            if (CHIndex[ID1].find(ID2) != CHIndex[ID1].end()) {//if found
                if (CHIndex[ID1][ID2].first != dis) {
                    cout << "Wrong label! " << ID1 << "(" << NodeOrder[ID1] << ") " << ID2 << "(" << NodeOrder[ID2]
                         << ") " << CHIndex[ID1][ID2].first << "(" << dis << ") " << CHIndex[ID1][ID2].second << "("
                         << cnt << ")" << endl;
//                    exit(1);
                } else if (CHIndex[ID1][ID2].second != cnt) {
                    cout << "Wrong cnt. " << ID1 << "(" << NodeOrder[ID1] << ") " << ID2 << "(" << NodeOrder[ID2]
                         << ") " << CHIndex[ID1][ID2].first << "(" << dis << ") " << CHIndex[ID1][ID2].second << "("
                         << cnt << ")" << endl;
                }
            } else {
                cout << "Wrong. Not found. " << ID1 << " " << ID2 << " " << dis << endl;
                exit(1);
            }
        }
    }
...
//    }
    ...
    for (int i = 0; i < Tree.size(); ++i) {
        ...
        for (auto it = Tree[i].vert.begin(); it != Tree[i].vert.end(); ++it) {
            ...
            if (CHIndex[ID1].find(ID2) != CHIndex[ID1].end()) {//if found
                if (CHIndex[ID1][ID2].first != dis) {
                    cout << "Wrong label! " << ID1 << "(" << PartiTags[ID1].first << ") " << ID2 << "("
                         << PartiTags[ID2].first << ") " << CHIndex[ID1][ID2].first << "(" << dis << ") "
                         << CHIndex[ID1][ID2].second << "(" << cnt << ")" << endl;
//                    exit(1);
                } else if (CHIndex[ID1][ID2].second != cnt) {
                    cout << "Wrong cnt. " << ID1 << "(" << PartiTags[ID1].first << ") " << ID2 << "("
                         << PartiTags[ID2].first << ") " << CHIndex[ID1][ID2].first << "(" << dis << ") "
                         << CHIndex[ID1][ID2].second << "(" << cnt << ")" << endl;
                }
            } else {
...
void Graph::RandomUpdateThroughputTest(string updateFile, int batchNum, int batchSize, int batchInterval) {
    ...
    IF.close();
    cout << "Update batch number: " << batchNum << " ; batch size: " << batchSize << " ; Batch interval: "
         << batchInterval << endl;
...
void Graph::RealUpdateThroughputTestQueueModel(string updateFile, int batchNum, double T_r, int workerNum,
                                               int regionNum) {//T_r is in seconds
    bool ifDebug = false;
//    ifDebug=true;
    int runtimes = 10000;
//    runtimes=1000;
    ...
    cout << "Update batch number: " << batchNumMax << " (" << batchNum << ") ; Average batch size: " << aveBatchSize
         << " ; Maximal batch size: " << maxBatchSize << " ; Minimal batch size: " << minBatchSize
         << " ; Batch interval: " << batchInterval << endl;
...
//    int clusterNum=4;
    if (algoChoice >= Index_DVPL && algoChoice <= static_cast<int>(HTSPIndex::DVPLwoRVO)) {
        if (algoChoice == static_cast<int>(HTSPIndex::DVPLwoR) ||
            algoChoice == static_cast<int>(HTSPIndex::DVPLwoRV) ||
            algoChoice == static_cast<int>(HTSPIndex::DVPLwoRVO)) {
            regionNum = 1;
            cout << "region number is set to " << regionNum << endl;
        }
    ...
    double runT1 = 0, runT2 = 0;
    unsigned long long throughputNum = 0;
    if (algoChoice == Index_PostMHL || (algoChoice >= Index_DVPL && algoChoice <= static_cast<int>(HTSPIndex::MVCC_PostMHL))) {
        ProBeginVertexSetParti.assign(partiNum, vector<int>());
        vertexIDChLParti.assign(partiNum, set<int>());
        ProBeginVertexSetPartiExtend.assign(partiNum, vector<int>());
        if (algoChoice >= Index_DVPL && algoChoice <= static_cast<int>(HTSPIndex::DVPLwoRVO)) {
...
//    batchNum=2;
    ...
    if (batchNum <= 4 || batchNum == batchNumMax) {
        ...
        sampleBatch = SampleBatches(
                sourcePath + "/" + dataset + "_20160105_" + to_string(batchInterval) + ".batchUpdatesInfo", batchNum);
    }
...
//                            }
            ...
            if (uEdges.find(make_pair(ID1, ID2)) == uEdges.end()) {//if not found
                ...
                cout << "Wrong. Find. " << ID1 << " " << ID2 << " " << newW << " " << uEdges[make_pair(ID1, ID2)] << " "
                     << oldW << endl;
                exit(1);
            }
        ...
        cout << "Batch " << batch_i << " . uNum: " << wBatchDec.size() + wBatchInc.size() << "("
             << batchUpdates[batch_i].size() << ") ; Decrease update number: " << wBatchDec.size()
             << " ; Increase update number: " << wBatchInc.size() << endl;
...
//            cout<<"Estimated throughput: "<<resultE.first<<" ; fastest available query time: "<<resultE.second*1000<<" ("<<fastQT<<") ms"<<endl;
            if (algoChoice == static_cast<int>(HTSPIndex::MVCC_PostMHL)) {
                throughputNum = ThroughputSimulateRealMVCC(stageQueryThis, stageUpdateT, batchInterval, T_r, batchInterval,
                                                       fastQT, workerNum);//2 batches update
            } else {
                throughputNum = ThroughputSimulateReal(stageQueryThis, stageUpdateT, batchInterval, T_r, batchInterval,
                                                       fastQT, workerNum);//2 batches update
            }
    ...
    for (int i = 0; i < stageQueryT.size(); ++i) {
        if ((algoChoice >= Index_DVPL && algoChoice <= static_cast<int>(HTSPIndex::DVPLwoRVO)) || algoChoice == Index_PostMHL || algoChoice == static_cast<int>(HTSPIndex::MVCC_PostMHL)) {
            if (i <= PCH_No) {
                ...
                cout << "Average query time of Q-Stage " << i << " : " << get_mean(stageQueryT[i]) * 1000 << " ms"
                     << endl;
            }
        } else {
            cout << "Average query time of Q-Stage " << i + 1 << " : " << get_mean(stageQueryT[i]) * 1000 << " ms"
...
//function of testing the throughput on random updates
void Graph::RandomUpdateThroughputTestQueueModel(int batchNum, int batchSize, int batchInterval, double T_r,
                                                 int workerNum) {//T_r is in seconds
    bool ifDebug = false;
//    ifDebug=true;
    int runtimes = 10000;
//    runtimes=1000;
...
//    AverageStagePerformance(batchNum,stageUpdateT,stageQueryT);
    ...
    for (int i = 0; i < aveDuration.size() - 1; ++i) {
        cout << "Average update time of U-Stage " << i + 2 << " : " << aveDuration[i] / batchNum << " s ; query time: "
             << get_mean(stageQueryT[i]) * 1000 << " ms" << endl;
    }


    pair<double, double> resultE = ThroughputEstimate(stageQueryT, stageUpdateT, T_r, batchInterval);
    cout << "Estimated throughput: " << resultE.first << " ; fastest available query time: " << resultE.second * 1000
         << " ms" << endl;
    throughputNum = ThroughputSimulate(stageQueryT, stageUpdateT, batchInterval * batchNum, T_r, batchInterval,
                                       resultE.second, workerNum);

    cout << "\nPartiNum: " << partiNum << ". Throughput: " << throughputNum << " ; Average batch update Time: "
         << updateTime / batchNum << " s." << endl;
}
...
void
Graph::SPThroughputTest(int updateType, bool ifBatch, int batchNum, int batchSize, int batchInterval, int runtimes) {
    ...
    cout << "Update batch: " << batchNum << " ; Batch size: " << batchSize << " ; Batch interval: " << batchInterval
         << endl;
...
//                Graph g2=*this;
                ...
                for (int u = 0; u < batchNum; u++) {
                    ...
                    for (int i = 0; i < batchSize; ++i) {
                        ...
                        newW = oldW * 0.5;
                        if (newW < 1) {
                            cout << "New edge weight is not positive! " << ID1 << " " << ID2 << " " << oldW << " "
                                 << newW << endl;
                            exit(1);
                        }
                        if (ifDebug) {
...
//                        g2.CorrectnessCheckH2H(100);
                ...
                cout << "\nPartiNum: " << partiNum << ". Overall throughput: " << throughputNum
                     << " ; Average throughput: " << throughputNum / batchNum
                     << " ; Average Decrease batch update Time: " << runT1 / batchNum << " s." << endl;
            }
            break;
        }
        case 2: {
...
//                    exit(0);
                ...
                cout << "\nPartiNum: " << partiNum << ". Overall throughput: " << throughputNum
                     << " ; Average throughput: " << throughputNum / batchNum
                     << " ; Average Increase batch update Time: " << runT2 / batchNum << " s." << endl;
            }
...
void Graph::StagePerformanceShow(int batchNum, vector<double> &stageUpdateT, vector<double> &stageQueryT) {
    if (algoChoice == 1) {
        cout << "\nStage 1 (BiDijkstra). Average update time: " << stageUpdateT[Dijk] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[Dijk] / batchNum << " ms." << endl;
        cout << "Stage 2 (CH). Average update time: " << stageUpdateT[CH] / batchNum << " s; average query time: "
             << 1000 * stageQueryT[CH] / batchNum << " ms." << endl;
        cout << "Stage 3 (H2H). Average duration: " << stageUpdateT[H2H] / batchNum << " s; average query time: "
             << 1000 * stageQueryT[H2H] / batchNum << " ms." << endl;
    } else if (algoChoice == 3) {
        cout << "\nStage 1 (BiDijkstra). Average update time: " << stageUpdateT[Dijk] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[Dijk] / batchNum << " ms." << endl;
        cout << "Stage 2 (PCH). Average update time: " << stageUpdateT[PCH_No] / batchNum << " s; average query time: "
             << 1000 * stageQueryT[PCH_No] / batchNum << " ms." << endl;
        cout << "Stage 3 (No-boundary PMHL). Average update time: " << stageUpdateT[PH2H_No] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[PH2H_No] / batchNum << " ms." << endl;
        cout << "Stage 4 (Post-boundary PMHL). Average update time: " << stageUpdateT[PH2H_Post] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[PH2H_Post] / batchNum << " ms." << endl;
        cout << "Stage 5 (Cross-boundary PMHL). Average duration: " << stageUpdateT[PH2H_Cross] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[PH2H_Cross] / batchNum << " ms." << endl;
    } else if (algoChoice == 5) {
        cout << "\nStage 1 (BiDijkstra). Average update time: " << stageUpdateT[Dijk] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[Dijk] / batchNum << " ms." << endl;
        cout << "Stage 2 (PCH). Average update time: " << stageUpdateT[PCH_No] / batchNum << " s; average query time: "
             << 1000 * stageQueryT[PCH_No] / batchNum << " ms." << endl;
        cout << "Stage 3 (Post-boundary PostMHL). Average update time: " << stageUpdateT[PH2H_Post] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[PH2H_Post] / batchNum << " ms." << endl;
        cout << "Stage 4 (Cross-boundary PostMHL). Average duration: " << stageUpdateT[PH2H_Cross] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[PH2H_Cross] / batchNum << " ms." << endl;
    }
}
...
void Graph::AverageStagePerformance(int batchNum, vector<double> &stageUpdateT, vector<double> &stageQueryT) {
    if (algoChoice == 1) {
        cout << "\nStage 1 (BiDijkstra). Average update time: " << stageUpdateT[Dijk] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[Dijk] << " ms." << endl;
        cout << "Stage 2 (CH). Average update time: " << stageUpdateT[CH] / batchNum << " s; average query time: "
             << 1000 * stageQueryT[CH] << " ms." << endl;
        cout << "Stage 3 (H2H). Average duration: " << stageUpdateT[H2H] / batchNum << " s; average query time: "
             << 1000 * stageQueryT[H2H] << " ms." << endl;
    } else if (algoChoice == 3) {
        cout << "\nStage 1 (BiDijkstra). Average update time: " << stageUpdateT[Dijk] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[Dijk] << " ms." << endl;
        cout << "Stage 2 (PCH). Average update time: " << stageUpdateT[PCH_No] / batchNum << " s; average query time: "
             << 1000 * stageQueryT[PCH_No] << " ms." << endl;
        cout << "Stage 3 (No-boundary PMHL). Average update time: " << stageUpdateT[PH2H_No] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[PH2H_No] << " ms." << endl;
        cout << "Stage 4 (Post-boundary PMHL). Average update time: " << stageUpdateT[PH2H_Post] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[PH2H_Post] << " ms." << endl;
        cout << "Stage 5 (Cross-boundary PMHL). Average duration: " << stageUpdateT[PH2H_Cross] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[PH2H_Cross] << " ms." << endl;
    } else if (algoChoice == 5) {
        cout << "\nStage 1 (BiDijkstra). Average update time: " << stageUpdateT[Dijk] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[Dijk] << " ms." << endl;
        cout << "Stage 2 (PCH). Average update time: " << stageUpdateT[PCH_No] / batchNum << " s; average query time: "
        ...
        cout << "Stage 4 (Post-boundary PostMHL). Average update time: " << stageUpdateT[PH2H_Post] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[PH2H_Post] << " ms." << endl;
        cout << "Stage 5 (Cross-boundary PostMHL). Average duration: " << stageUpdateT[PH2H_Cross] / batchNum
             << " s; average query time: " << 1000 * stageQueryT[PH2H_Cross] << " ms." << endl;
    }
}
...
void Graph::PMHLBatchUpdateDec(vector<pair<pair<int, int>, pair<int, int>>> &wBatch, int batch_i, double &runT1) {
    ...
    Timer tt2;
    tt.start();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatch;
    partiBatch.clear();
    ...
    if (!partiBatch.empty()) {
        if (threadnum == 1) {
            ...
            if (partiBatch.size() > threadnum) {
                ...
                boost::thread_group thread;
                for (auto j = 0; j < processID.size(); ++j) {
                    thread.add_thread(new boost::thread(&Graph::DecreasePartiBatchUpdateCheckCHV, this, processID[j],
                                                        boost::ref(partiBatch), boost::ref(overlayBatch), false,
                                                        boost::ref(updatedSCs)));
                }
                thread.join_all();
            } else {
                boost::thread_group thread;
                for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
                    int pid = it->first;
                    thread.add_thread(new boost::thread(&Graph::DecreasePartiBatchUpdateCheckCH, this, pid,
                                                        boost::ref(it->second), boost::ref(overlayBatch), false,
                                                        boost::ref(updatedSCs[pid])));//old
...
//                sm->wait();
                if (updateSCTrue.find(make_pair(bid1, bid2)) == updateSCTrue.end()) {
                    ...
                    cout << "More than one supportive vertices. " << bid1 << " " << bid2 << " "
                         << updateSCTrue[make_pair(bid1, bid2)].second << " " << newdis << endl;
...
//    cout<<"algoQuery: PCH-No"<<endl;
    ...
    if (threadnum == 1) {
        DecreaseOverlayBatchLabel(Tree, rank, heightMax, ProBeginVertexSetOverlay,
                                  vertexIDChLOverlay);//vector<Node> &Tree, vector<int> &rank, int heightMax, vector<int> &ProBeginVertexSet, set<int> &vertexIDChL
        if (!partiBatch.empty()) {
//        if(partiBatch.size()>threadnum){
//            cout<<"partiBatch is larger than thread number! "<<partiBatch.size()<<" "<<threadnum<< endl;
//        }
...
//            cout<<"partition "<<pid<<" "<<heightMaxs[pid]<< endl;
                DecreasePartiBatchLabel(Trees[pid], ranks[pid], heightMaxs[pid], ProBeginVertexSetParti[pid],
                                        vertexIDChLParti[pid]);
            }
        }
        ...
        if (partiBatch.size() + 1 > threadnum) {
            boost::thread_group thread;
            thread.add_thread(
                    new boost::thread(&Graph::DecreaseOverlayBatchLabel, this, boost::ref(Tree), boost::ref(rank),
                                      heightMax, boost::ref(ProBeginVertexSetOverlay), boost::ref(vertexIDChLOverlay)));
            ...
            for (auto j = 0; j < processID.size(); ++j) {
                thread.add_thread(
                        new boost::thread(&Graph::DecreasePartiBatchLabelV, this, processID[j], boost::ref(Trees),
                                          boost::ref(ranks), boost::ref(heightMaxs), boost::ref(ProBeginVertexSetParti),
                                          boost::ref(vertexIDChLParti)));
            }
            thread.join_all();
...
//        cout<<"Update Partition number: "<<partiBatch.size()<<endl;
                for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
                    int pid = it->first;
//            cout<<"partition "<<pid<<" "<<heightMaxs[pid]<< endl;
                    thread.add_thread(new boost::thread(&Graph::DecreasePartiBatchLabel, this, boost::ref(Trees[pid]),
                                                        boost::ref(ranks[pid]), heightMaxs[pid],
                                                        boost::ref(ProBeginVertexSetParti[pid]),
                                                        boost::ref(vertexIDChLParti[pid])));
                }
            }
            thread.join_all();
        }
...
//        cout<<"Update Partition number: "<<partiBatch.size()<<endl;
        ...
        for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
            int pid = it->first;
            thread.add_thread(
                    new boost::thread(&Graph::DecreasePartiBatchUpdateCheckCH, this, pid, boost::ref(it->second),
                                      boost::ref(overlayBatch), true, boost::ref(updatedSCs[pid])));
        }
        thread.join_all();
    }
...
void Graph::VPLBatchUpdateMixSchedule(vector<pair<pair<int, int>, pair<int, int>>> &wBatchDec,
                                      vector<pair<pair<int, int>, pair<int, int>>> &wBatchInc, int batch_i,
                                      double &runT1) {
    Timer tt;
    Timer tt2;
    Timer tt3;
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatchDec;
    partiBatchDec.clear();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatchInc;
    partiBatchInc.clear();
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> overlayBatchDecM;
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> overlayBatchIncM;
    vector<pair<pair<int, int>, pair<int, int>>> overlayBatchDec;
...
//                    cout<<"Parti update: "<<pid1<<". "<<a<<"("<<pid1<<") "<<b<<"("<<pid2<<") "<<newW<<endl;
                    ...
                    cout << "Wrong for this edge update. " << pid1 << " " << pid2 << endl;
                    exit(1);
                }
            }
        }
        ...
        for (auto it = clusterP[ci].begin(); it != clusterP[ci].end(); ++it) {
            if (overlayBatchDecM.find(*it) != overlayBatchDecM.end()) {
                for (int k = 0; k < overlayBatchDecM[*it].size(); ++k) {
                    ...
                    overlayBatchDecSchedule.emplace_back(overlayBatchDecM[*it][k].first,
                                                         overlayBatchDecM[*it][k].second);
                    for (int i = 0; i < Neighbor[a].size(); i++) {
                        if (Neighbor[a][i].first == b) {
//            cout<<Neighbor[a][i].second<<" "<<newW<<endl;
                            assert(oldW == Neighbor[a][i].second);
...
//            cout<<Neighbor[b][i].second<<" "<<newW<<endl;
            ...
            if (overlayBatchIncM.find(*it) != overlayBatchIncM.end()) {
                for (int k = 0; k < overlayBatchIncM[*it].size(); ++k) {
                    ...
                    overlayBatchIncSchedule.emplace_back(overlayBatchIncM[*it][k].first,
                                                         overlayBatchIncM[*it][k].second);
                    for (int i = 0; i < Neighbor[a].size(); i++) {
                        if (Neighbor[a][i].first == b) {
//            cout<<Neighbor[a][i].second<<" "<<newW<<endl;
                            assert(oldW == Neighbor[a][i].second);
...
//            cout<<Neighbor[b][i].second<<" "<<newW<<endl;
        ...
        cout << "partiDec size: " << partiBatchDecSchedule.size() << " ; partiInc size: "
             << partiBatchIncSchedule.size() << " ; overlayDec size: " << overlayBatchDecSchedule.size()
             << " ; overlayInc size: " << overlayBatchIncSchedule.size() << endl;
        tt2.start();
        innerAffectedParti.assign(partiNum, true);
        outerAffectedParti.assign(partiNum, false);
        AffectedParti.assign(partiNum, true);
        affectedParti.clear();
        affectedPartiInc.clear();
        overlayShortcutDec.clear();
        vUpdated.assign(node_num, false);
        ProBeginVertexSetOverlay.clear();
        vertexIDChLOverlay.clear();
        ProBeginVertexSetOverlayInc.clear();
...
//        VPLShortcutUpdate(partiBatchDecSchedule, partiBatchIncSchedule,overlayBatchDecSchedule,overlayBatchIncSchedule,tSCU);
        if (algoChoice != static_cast<int>(HTSPIndex::DVPLwoRV) && algoChoice != static_cast<int>(HTSPIndex::DVPLwoRVO)) {
            boost::thread_group thread1;
...
//            DecreaseOverlayBatchLabelVPL(Tree,rank,heightMax,ProBeginVertexSetOverlay,vertexIDChLOverlay);//top-down update for overlay
//            IncreaseOverlayBatchLabelVPL(Tree, rank, heightMax, ProBeginVertexSetOverlayInc, VidtoTNid);
            MixVPLOverlayLabelUpdateTopDown(overlayLDec, overlayLInc, Tree, rank,
                                            VidtoTNid);//correct version by top-down mechanism
            tt2.stop();
            tOverlay = tt2.GetRuntime();
            overlayUpdateT += tOverlay;
            cout << "Overlay label update time: " << tOverlay << " s." << endl;
            ...
            if (algoUpdate == PH2H_Post) {
                ...
                if (!partiBatchDecSchedule.empty() || !partiBatchIncSchedule.empty()) {
                    tt.start();
                    BoundShortcutsCheck(true,
                                        MixUpdate);// check which partition is affected and update the boundary shortcuts
                    tt.stop();
                    cout << "All-pair boundary check time: " << tt.GetRuntime() << " s. Decrease affectedParti size: "
                         << affectedParti.size() << " ; Increase affectedParti size: " << affectedPartiInc.size()
                         << endl;
                    ifRepaired.assign(partiNum, false);
                }
...
//                cout<<"Post-boundary and cross-boundary could be parallelized!!!"<<endl;
                    boost::thread_group thread;
                    thread.add_thread(new boost::thread(&Graph::MixRepair_PartiIndexVPLPost, this, true,
                                                        boost::ref(partiBatchDecSchedule),
                                                        boost::ref(partiBatchIncSchedule), boost::ref(tPost)));
//                thread.add_thread(new boost::thread(&Graph::MixRefreshCrossLabelsVPL, this, true, boost::ref(tCross), threadnum-max(affectedParti.size(),affectedPartiInc.size())));
                    thread.add_thread(
                            new boost::thread(&Graph::MixRefreshCrossLabelsVPL, this, true, boost::ref(tCross),
                                              threadnum - 2));
                    thread.join_all();
                    if (tPost < tCross) {
                        ...
                        cout << "!!! Cross-boundary update is faster than post-boundary update! " << tPost << " "
                             << tCross << endl;
...
void Graph::VPLShortcutUpdate(map<int, vector<pair<pair<int, int>, pair<int, int>>>> &partiBatchDecSchedule,
                              map<int, vector<pair<pair<int, int>, pair<int, int>>>> &partiBatchIncSchedule,
                              vector<pair<pair<int, int>, pair<int, int>>> &overlayBatchDecSchedule,
                              vector<pair<pair<int, int>, pair<int, int>>> &overlayBatchIncSchedule, double &tSc) {
    Timer tt;
    Timer tt2;
    tt2.start();
    // Step 1: Update shortcuts
...
//    tt.stop();
//    cout<<"Partition shortcut update time: "<<tt.GetRuntime()<<" s."<<endl;
    MixVPLBatchShortcutUpdateParti(partiBatchDecSchedule, partiBatchIncSchedule, overlayBatchDecSchedule,
                                   overlayBatchIncSchedule, true);///Mix update
...
//    DecreaseOverlayBatchVPL(overlayBatchDecSchedule,Tree,rank,heightMax,false);
//    IncreaseOverlayBatchVPL(overlayBatchIncSchedule,Tree,rank,heightMax,SCconNodesMT,VidtoTNid,false);
    MixVPLBatchShortcutUpdateOverlay(overlayBatchDecSchedule, overlayBatchIncSchedule, Tree, rank, heightMax,
                                     SCconNodesMT, VidtoTNid, false);
//    tt.stop();
//    cout<<"Overlay shortcut update time: "<<tt.GetRuntime()<<" s."<<endl;
    tt2.stop();
    tSc = tt2.GetRuntime();
...
//function for VPL's throughput test of mix update
void Graph::VPLBatchUpdateMix(vector<pair<pair<int, int>, pair<int, int>>> &wBatchDec,
                              vector<pair<pair<int, int>, pair<int, int>>> &wBatchInc, int batch_i, double &runT1) {
    Timer tt;
    Timer tt2;
    map<int, vector<pair<pair<int, int>, pair<int, int>>>> partiBatchDec;
    partiBatchDec.clear();
...
//            cout<<Neighbor[a][i].second<<" "<<newW<<endl;
                ...
                if (newW < Neighbor[a][i].second) {
                    ...
                    cout << "Wrong update. " << a << " " << b << " " << Neighbor[a][i].second << " " << newW << endl;
                    exit(1);
                }
                break;
            }
...
//            cout<<Neighbor[b][i].second<<" "<<newW<<endl;
                ...
                if (newW < Neighbor[b][i].second) {
                    ...
                    cout << "Wrong decrease update. " << a << " " << b << " " << Neighbor[a][i].second << " " << newW
                         << endl;
                    exit(1);
                }
...
//    if(threadnum<=affectedParti.size() && threadnum<=affectedPartiInc.size()){
        ...
        if (algoUpdate >= PH2H_Post) {
            ...
            DecreaseOverlayBatchLabelVPL(Tree, rank, heightMax, ProBeginVertexSetOverlay,
                                         vertexIDChLOverlay);//top-down update for overlay
            IncreaseOverlayBatchLabelVPL(Tree, rank, heightMax, ProBeginVertexSetOverlayInc, VidtoTNid);
            tt2.stop();
            tOverlay = tt2.GetRuntime();
...
//            cout<<"Decrease BP-Flag update time: "<<tt.GetRuntime()<<" s."<<endl;
            ...
            boost::thread_group thread;
            thread.add_thread(
                    new boost::thread(&Graph::MixRepair_PartiIndexVPLPost, this, true, boost::ref(partiBatchDec),
                                      boost::ref(partiBatchInc), boost::ref(tPost)));
            thread.add_thread(new boost::thread(&Graph::MixRefreshCrossLabelsVPL, this, true, boost::ref(tCross),
                                                threadnum - max(affectedParti.size(), affectedPartiInc.size())));
            thread.join_all();
            if (tPost < tCross) {
                stageDurations[PCH_No] = stageDurations[PCH_No] + tOverlay + tPost;
                stageDurations[PH2H_Post] = stageDurations[PH2H_Post] + tCross - tPost;
            } else {
                cout << "!!! Cross-boundary update is faster than post-boundary update! " << tPost << " " << tCross
                     << endl;
                stageDurations[PCH_No] = stageDurations[PCH_No] + tOverlay + tCross;
//            stageDurations[PH2H_Post]=0;
            }
...
//        Repair_PartiIndexPostMHLPost(false, false, partiBatch);//post
            ...
            tt2.start();
            DecreaseOverlayBatchLabelPostMHL(Tree, rank, heightMax, ProBeginVertexSetOverlay,
                                             vertexIDChLOverlay);//top-down update for overlay
            tt2.stop();
            tOverlay = tt2.GetRuntime();
            cout << "Overlay label update time: " << tOverlay << " s." << endl;
            ...
            boost::thread_group thread;
            thread.add_thread(
                    new boost::thread(&Graph::Repair_PartiIndexVPLPost, this, true, false, boost::ref(partiBatch),
                                      boost::ref(tPost)));
            thread.add_thread(new boost::thread(&Graph::RefreshCrossLabelsVPL, this, true, false, boost::ref(tCross),
                                                threadnum - affectedParti.size()));
            thread.join_all();
            if (tPost < tCross) {
                stageDurations[PCH_No] = stageDurations[PCH_No] + tOverlay + tPost;
                stageDurations[PH2H_Post] = stageDurations[PH2H_Post] + tCross - tPost;
...
//                    cout<<"Parti update: "<<pid1<<". "<<a<<"("<<pid1<<") "<<b<<"("<<pid2<<") "<<newW<<endl;
                    ...
                    cout << "Wrong for this edge update. " << a << "(" << pid1 << ") " << b << "(" << pid2 << ")"
                         << endl;
                    exit(1);
                }
...
//        Repair_PartiIndexPostMHLPost(false, true, partiBatch);//post
            tt2.stop();
            cout << "Post-boundary index update time: " << tt2.GetRuntime() << " s; Boundary array update: "
                 << tBoundary << " s; Ancestor array update: " << tAncestor << " s." << endl;
            stageDurations[PCH_No] = stageDurations[PCH_No] + tt2.GetRuntime() + toverlay;
            if (algoUpdate == PH2H_Cross) {
                tt2.start();
...
//        Repair_PartiIndexPostMHLPost(false, true, partiBatch);//post
            ...
            boost::thread_group thread;
            thread.add_thread(
                    new boost::thread(&Graph::Repair_PartiIndexVPLPost, this, true, true, boost::ref(partiBatch),
                                      boost::ref(tPost)));
            thread.add_thread(new boost::thread(&Graph::RefreshCrossLabelsVPL, this, true, true, boost::ref(tCross),
                                                threadnum - affectedPartiInc.size()));
            thread.join_all();
            if (tPost < tCross) {
                stageDurations[PCH_No] = stageDurations[PCH_No] + tOverlay + tPost;
                stageDurations[PH2H_Post] = stageDurations[PH2H_Post] + tCross - tPost;
...
//            RefreshExtensionLabelsPostMHL(false, false);//extend, single-thread
        ...
        if (algoUpdate == PH2H_Post) {
            ...
            overlayUpdateT += tOverlay;
            boost::thread_group thread;
            thread.add_thread(new boost::thread(&Graph::Repair_PartiIndexPostMHLPost, this, true, DecUpdate,
                                                boost::ref(partiBatch), boost::ref(tPost)));
            thread.add_thread(
                    new boost::thread(&Graph::RefreshExtensionLabelsPostMHL, this, true, false, boost::ref(tCross),
                                      threadnum - 2));//threadnum-affectedParti.size()
            thread.join_all();
            if (tPost < tCross) {
                stageDurations[PCH_No] = stageDurations[PCH_No] + tOverlay + tPost;
                stageDurations[PH2H_Post] = stageDurations[PH2H_Post] + tCross - tPost;
...
//            RefreshExtensionLabelsPostMHL(false, true);//extend, single-thread
        ...
        if (algoUpdate == PH2H_Post) {
            ...
            overlayUpdateT += tOverlay;
            boost::thread_group thread;
            thread.add_thread(new boost::thread(&Graph::Repair_PartiIndexPostMHLPost, this, true, IncUpdate,
                                                boost::ref(partiBatch), boost::ref(tPost)));
            thread.add_thread(
                    new boost::thread(&Graph::RefreshExtensionLabelsPostMHL, this, true, true, boost::ref(tCross),
                                      threadnum - 2));//threadnum-affectedParti.size()
            thread.join_all();
            if (tPost < tCross) {
                stageDurations[PCH_No] = stageDurations[PCH_No] + tOverlay + tPost;
    ...
    if (!partiBatch.empty()) {
        if (threadnum == 1) {
            ...
            if (partiBatch.size() > threadnum) {
                ...
                boost::thread_group thread;
                for (auto j = 0; j < processID.size(); ++j) {
                    thread.add_thread(new boost::thread(&Graph::IncreasePartiBatchUpdateCheckCHV, this, processID[j],
                                                        boost::ref(partiBatch), boost::ref(overlayBatch), false,
                                                        boost::ref(updatedSCs)));
                }
                thread.join_all();
                ...
                for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
                    int pid = it->first;
                    thread.add_thread(new boost::thread(&Graph::IncreasePartiBatchUpdateCheckCH, this, pid,
                                                        boost::ref(it->second), boost::ref(overlayBatch), false,
...
//                sm->notify();
                ...
                cout << "Something wrong happens. " << bid1 << "(" << PartiTag[bid1].first << ") " << bid2 << "("
                     << PartiTag[bid2].first << ") : " << newdis << " " << olddis << endl;
                exit(1);
            }
        }
    }
//    cout<<"updateSCTrue size: "<<updateSCTrue.size()<<" "<< updateSCSize<<endl;
    ...
    if (threadnum == 1) {
        ...
        if (!partiBatch.empty()) {
            for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
                int pid = it->first;
                IncreasePartiBatchLabel(Trees[pid], ranks[pid], heightMaxs[pid], ProBeginVertexSetParti[pid],
                                        VidtoTNidP);//vector<Node> &Tree, vector<int> &rank, int heightMax, int& checknum, vector<int>& ProBeginVertexSet, vector<vector<int>> &VidtoTNid
            }
        }
    } else {
        if (partiBatch.size() + 1 > threadnum) {
            boost::thread_group thread;
            thread.add_thread(
                    new boost::thread(&Graph::IncreaseOverlayBatchLabel, this, boost::ref(Tree), boost::ref(rank),
                                      heightMax, boost::ref(ProBeginVertexSetOverlay), boost::ref(VidtoTNid)));
            vector<vector<int>> processID;
            processID.assign(threadnum - 1, vector<int>());
            vector<int> vertices;
            for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
            ...
            for (auto j = 0; j < processID.size(); ++j) {
                thread.add_thread(
                        new boost::thread(&Graph::IncreasePartiBatchLabelV, this, processID[j], boost::ref(Trees),
                                          boost::ref(ranks), boost::ref(heightMaxs), boost::ref(ProBeginVertexSetParti),
                                          boost::ref(VidtoTNidP)));
            }
            thread.join_all();
        } else {
            boost::thread_group thread;
            ...
            if (!partiBatch.empty()) {
                for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
                    int pid = it->first;
                    thread.add_thread(new boost::thread(&Graph::IncreasePartiBatchLabel, this, boost::ref(Trees[pid]),
                                                        boost::ref(ranks[pid]), heightMaxs[pid],
                                                        boost::ref(ProBeginVertexSetParti[pid]), boost::ref(
                                    VidtoTNidP)));//vector<Node> &Tree, vector<int> &rank, int heightMax, int& checknum, vector<int>& ProBeginVertexSet, vector<vector<int>> &VidtoTNid
                }
            }
            thread.join_all();
...
//    cout<<"Before parti CH, size of overlayBatch: "<<overlayBatch.size()<<endl;
    if (!partiBatch.empty()) {
        ...
        for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
            int pid = it->first;
            thread.add_thread(
                    new boost::thread(&Graph::IncreasePartiBatchUpdateCheckCH, this, pid, boost::ref(it->second),
                                      boost::ref(overlayBatch), true, boost::ref(updatedSCs[pid])));
...
//            cout<<Neighbor[b][i].second<<" "<<newW<<endl;
    ...
    if (!partiBatch.empty()) {
        ...
        for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
            int pid = it->first;
            thread.add_thread(
                    new boost::thread(&Graph::DecreasePartiBatchUpdateCheck, this, pid, boost::ref(it->second),
                                      boost::ref(overlayBatch)));
        }
        thread.join_all();
    }
...
//            cout<<"Parti update. "<<a<<"("<<pid1<<") "<<b<<"("<<pid2<<") "<<oldW<<" "<<newW<<endl;
    ...
    if (!partiBatch.empty()) {
        ...
        for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
            int pid = it->first;
            thread.add_thread(
                    new boost::thread(&Graph::IncreasePartiBatchUpdateCheck, this, pid, boost::ref(it->second),
                                      boost::ref(overlayBatch)));
        }
        thread.join_all();
        ...
        for (int i = 0; i < Neighbor[a].size(); i++) {
            if (Neighbor[a][i].first == b) {
                if (newW < Neighbor[a][i].second) {
                    ...
                    cout << "Invalid update. " << a << " " << b << " " << Neighbor[a][i].second << " " << newW << endl;
                    exit(1);
                }
                break;
            }
        ...
        for (int i = 0; i < Neighbor[b].size(); i++) {
            if (Neighbor[b][i].first == a) {
                if (newW < Neighbor[b][i].second) {
                    ...
                    cout << "Invalid update. " << a << " " << b << " " << Neighbor[b][i].second << " " << newW << endl;
                    exit(1);
                }
                break;
            }
...
//	NodeOrderss.assign(NodeOrder.begin(),NodeOrder.end());
    ...
    SCre.assign(node_num, set<OrderCompp>());//{vertexID, set<int>}
    set<OrderCompp> OC;
    OC.clear();//the lower-order vertex of the affected shortcut, vertexID in decreasing node order
    ...
    for (int k = 0; k < wBatch.size(); k++) {
        ...
        if (oldW < newW) {
            ...
            cout << "Wrong update." << oldW << " " << newW << endl;
            exit(1);
        }
    }
...
//get the new value of shortcut
            ...
            if (newCw <= Cw) {
                ...
                for (int i = 0; i < Tree[rank[ProID]].vert.size(); i++) {
                    if (Tree[rank[ProID]].vert[i].first == Cid) {
                        cout << Tree[rank[ProID]].vert[i].second.first << "(" << newCw << ") "
                             << Tree[rank[ProID]].vert[i].second.second << "(" << countwt << ")";
                    }
                }
                cout << endl;
...
void Graph::eachNodeProcessIncrease1H2H(int children, vector<int> &line, int &changelabel) {
    ...
    for (int i = 0; i < Tree[children].dis.size(); i++) {
        if (Tree[children].cnt[i] <=
            0) {//if the distance label to i-th ancestor should be maintained, cnt may less than actual value
//        if(true){
            changelabel += 1;
            ...
            //chidlID
            for (int k = 0;
                 k < VidtoTNid[childID].size(); k++) {//for the tree node that contains childID as vert element
                PID = VidtoTNid[childID][k];
                if (Tree[PID].FN[childH] &&
                    Tree[PID].dis[i] == disBF + Tree[PID].dis[childH]) {//if label is from shortcut
                    Tree[PID].cnt[i] -= 1;
                }
            }
...
//                if(Tree[PID].height>Tree[children].height){///modified for correctness, PID may not be the descendant of children
                if (Tree[PID].height > Tree[children].height && Tree[PID].vAncestor[childH] ==
                                                                childID) {///modified for correctness, PID may not be the descendant of children
                    if (PID > Tree.size()) {
                        cout << "PID error! " << PID << " " << Tree.size() << endl;
                        exit(1);
                    }
                    if (childH > Tree[PID].dis.size()) {
                        cout << "childH error! " << childH << " " << Tree[PID].dis.size() << ": " << children << "("
                             << Tree[children].height << ") " << PID << "(" << Tree[PID].height << ")" << endl;
                        exit(1);
                    }
...
unsigned long long Graph::EffiCheckThroughput(vector<pair<int, int>> &ODpair, Timer &tRecord, int batchInterval,
                                              unsigned long long &throughputNum) {
    bool ifDebug = false;
//    ifDebug=true;
    int s, t;
...
//                d2 = Astar(s, t, Neighbor);
            ...
            if (ifDebug) {
                ...
                d1 = Dijkstra(s, t, Neighbor);
                if (d1 != d2) {
                    cout << "Effi Test. Incorrect! Algorithm " << algoQuery << ": " << s << " " << t << " " << d2 << " "
                         << d1 << endl;
                    exit(1);
                }
            }
        ...
        cout << endl;
        cout << "Stage 1 (Dijkstra): Duration: " << runT0 << " s; Throughput number: " << runtimes0 << " ; query time: "
             << 1000 * runT0 / runtimes0 << " ms." << endl;
        cout << "Stage 2 (CH): Duration: " << runT1 << " s; Throughput number: " << runtimes1 << " ; query time: "
             << 1000 * runT1 / runtimes1 << " ms." << endl;
        cout << "Stage 3 (H2H): Duration: " << runT2 << " s; Throughput number: " << runtimes2 << " ; query time: "
             << 1000 * runT2 / runtimes2 << " ms." << endl;
...
//                d2 = Astar(s, t, Neighbor);
            ...
            if (ifDebug) {
                ...
                d1 = Dijkstra(s, t, Neighbor);
                if (d1 != d2) {
                    cout << "Effi Test. Incorrect! Algorithm " << algoQuery << ": " << s << " (" << PartiTag[s].first
                         << ", " << PartiTag[s].second << ") " << t << " (" << PartiTag[t].first << ", "
                         << PartiTag[t].second << ") " << d2 << " " << d1 << endl;
                    exit(1);
                }
            }
        }
        ...
        cout << "Stage 1 (Dijkstra): Duration: " << runT0 << " s; Throughput number: " << runtimes0 << " ; query time: "
             << 1000 * runT0 / runtimes0 << " ms." << endl;
        cout << "Stage 2 (No-boundary): Duration: " << runT1 << " s; Throughput number: " << runtimes1
             << " ; query time: " << 1000 * runT1 / runtimes1 << " ms." << endl;
        cout << "Stage 3 (Post-boundary): Duration: " << runT2 << " s; Throughput number: " << runtimes2
             << " ; query time: " << 1000 * runT2 / runtimes2 << " ms." << endl;
        cout << "Stage 4 (Extension): Duration: " << runT3 << " s; Throughput number: " << runtimes3
             << " ; query time: " << 1000 * runT3 / runtimes3 << " ms." << endl;
...
//                d2 = Astar(s, t, Neighbor);
        ...
        cout << "Stage 1 (Dijkstra): Duration: " << runT0 << " s; Throughput number: " << runtimes0 << " ; query time: "
             << 1000 * runT0 / runtimes0 << " ms." << endl;
        cout << "Stage 2 (PCH-No): Duration: " << runT1 << " s; Throughput number: " << runtimes1 << " ; query time: "
             << 1000 * runT1 / runtimes1 << " ms." << endl;
        cout << "Stage 3 (PH2H-No): Duration: " << runT2 << " s; Throughput number: " << runtimes2 << " ; query time: "
             << 1000 * runT2 / runtimes2 << " ms." << endl;
        cout << "Stage 4 (PH2H-Post): Duration: " << runT4 << " s; Throughput number: " << runtimes4
             << " ; query time: " << 1000 * runT4 / runtimes4 << " ms." << endl;
        cout << "Stage 5 (PH2H-Extend): Duration: " << runT5 << " s; Throughput number: " << runtimes5
             << " ; query time: " << 1000 * runT5 / runtimes5 << " ms." << endl;
...
//                d2 = Astar(s, t, Neighbor);
        ...
        cout << "Stage 2 (PCH-No): Duration: " << runT1 << " s; Throughput number: " << runtimes1 << " ; query time: "
             << 1000 * runT1 / runtimes1 << " ms." << endl;
        cout << "Stage 3 (PH2H-Post): Duration: " << runT4 << " s; Throughput number: " << runtimes4
             << " ; query time: " << 1000 * runT4 / runtimes4 << " ms." << endl;
        cout << "Stage 4 (PH2H-Extend): Duration: " << runT5 << " s; Throughput number: " << runtimes5
             << " ; query time: " << 1000 * runT5 / runtimes5 << " ms." << endl;
...
//                d2 = Astar(s, t, Neighbor);
            ...
            if (ifDebug) {
                ...
                d1 = Dijkstra(s, t, Neighbor);
                if (d1 != d2) {
                    cout << "Effi Test. Incorrect! Algorithm " << algoQuery << ": " << s << " (" << PartiTags[s].first
                         << ") " << t << " (" << PartiTags[t].first << ") " << d2 << " " << d1 << endl;
                    QueryPostMHLDebug(s, t);
                    exit(1);
                }
            }
...
//                    DecreaseBatch(wBatch);
                ...
                for (int u = 0; u < updateBatch; u++) {
                    ...
                    newW = oldW * 0.5;
                    if (newW < 1) {
                        cout << "New edge weight is not positive! " << ID1 << " " << ID2 << " " << oldW << " " << newW
                             << endl;
                        exit(1);
                    }
                    if (ifDebug) {
...
//                    QueryPartiPartiExtDebug(107830,104726);
                ...
                for (int u = 0; u < updateBatch; u++) {
                    ...
                    newW = oldW * 2;
                    if (ifDebug) {
                        cout << "Batch " << u << ": " << ID1 << "(" << PartiTag[ID1].first << ") " << ID2 << "("
                             << PartiTag[ID2].first << ") " << oldW << " " << newW << endl;
                    }
...
void Graph::DecreasePartiBatchUpdateCheck(int pid, vector<pair<pair<int, int>, pair<int, int>>> &wBatch,
                                          vector<pair<pair<int, int>, pair<int, int>>> &weightOverlay) {
...
//                NeighborsOverlay[bid2][bid1]=newdis;
                sm->wait();
                weightOverlay.emplace_back(make_pair(bid1, bid2), make_pair(olddis,
...
void Graph::DecreasePartiBatchUpdateCheckCH(int pid, vector<pair<pair<int, int>, pair<int, int>>> &wBatch,
                                            vector<pair<pair<int, int>, pair<int, int>>> &weightOverlay, bool ifOpt,
                                            vector<pair<pair<int, int>, int>> &updatedSC) {
    //partition batch decrease update
//    vector<pair<pair<int,int>,int>> updatedSC;
    ...
    if (ifOpt) {
        DecreasePartiBatchForOpt(pid, wBatch, NeighborsParti, Trees[pid], ranks[pid], heightMaxs[pid], updatedSC, false,
                                 false);
    } else {
...
//partition update PCH
void
Graph::DecreasePartiBatchUpdateCheckCHV(vector<int> p, map<int, vector<pair<pair<int, int>, pair<int, int>>>> &wBatch,
                                        vector<pair<pair<int, int>, pair<int, int>>> &weightOverlay, bool ifOpt,
                                        vector<vector<pair<pair<int, int>, int>>> &updatedSC) {
    //partition batch decrease update
//    vector<pair<pair<int,int>,int>> updatedSC;
    ...
    for (int i = 0; i < p.size(); ++i) {
        int pid = p[i];
        if (ifOpt) {
            DecreasePartiBatchForOpt(pid, wBatch[pid], NeighborsParti, Trees[pid], ranks[pid], heightMaxs[pid],
                                     updatedSC[pid], false, false);
        } else {
            DecreasePartiBatch(pid, wBatch[pid], NeighborsParti, Trees[pid], ranks[pid], heightMaxs[pid],
                               updatedSC[pid], false);
        }
    }
}
...
void Graph::DecreasePartiBatchUpdateCheckPostMHL(map<int, vector<pair<pair<int, int>, pair<int, int>>>> &partiBatch,
                                                 vector<pair<pair<int, int>, pair<int, int>>> &overlayBatch,
                                                 bool ifParallel) {
    //partition batch decrease update
    vector<vector<pair<pair<int, int>, int>>> updatedSCs;
    ...
    if (ifParallel) {
        if (!partiBatch.empty()) {
            if (partiBatch.size() > threadnum) {
                ...
                boost::thread_group thread;
                for (auto j = 0; j < processID.size(); ++j) {
                    thread.add_thread(new boost::thread(&Graph::DecreasePartiBatchPostMHLShortcutV, this, processID[j],
...
//            cout<<"Update Partition number: "<<partiBatch.size()<<endl;
                ...
                for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
                    int pid = it->first;
                    thread.add_thread(new boost::thread(&Graph::DecreasePartiBatchPostMHLShortcut, this, pid,
                                                        boost::ref(it->second), boost::ref(Tree), boost::ref(rank),
                                                        heightMax, boost::ref(updatedSCs[pid])));
                }
                thread.join_all();
            }
...
//            sm->notify();
            ...
            cout << "Something wrong happens. " << bid1 << " " << bid2 << " " << olddis << " " << newdis << endl;
            exit(1);
        }
    }
...
void Graph::DecreasePartiBatchUpdateCheckVPL(map<int, vector<pair<pair<int, int>, pair<int, int>>>> &partiBatch,
                                             vector<pair<pair<int, int>, pair<int, int>>> &overlayBatch,
                                             bool ifParallel) {
    //partition batch decrease update
    if (!partiBatch.empty()) {
        ...
        if (ifParallel & partiBatch.size() > 1) {
            if (partiBatch.size() > threadnum) {
                ...
                boost::thread_group thread;
                for (auto j = 0; j < processID.size(); ++j) {
                    thread.add_thread(new boost::thread(&Graph::DecreasePartiBatchVPLShortcutV, this, processID[j],
                                                        boost::ref(partiBatch), boost::ref(Tree), boost::ref(rank),
                                                        heightMax, boost::ref(updatedSCs)));
                }
                thread.join_all();
...
//            cout<<"Update Partition number: "<<partiBatch.size()<<endl;
                ...
                for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
                    int pid = it->first;
                    thread.add_thread(
                            new boost::thread(&Graph::DecreasePartiBatchVPLShortcut, this, pid, boost::ref(it->second),
...
//            sm->notify();
                ...
                cout << "dec Something wrong happens. " << bid1 << " " << bid2 << " " << olddis << " " << newdis
                     << endl;
                exit(1);//not for mix
            }
        }
...
//for PH2H
void Graph::IncreasePartiBatchUpdateCheck(int pid, vector<pair<pair<int, int>, pair<int, int>>> &wBatch,
                                          vector<pair<pair<int, int>, pair<int, int>>> &weightOverlay) {
    //partition batch Increase update
    vector<pair<pair<int, int>, int>> updatedSC;

    IncreasePartiBatch(pid, wBatch, NeighborsParti, Trees[pid], ranks[pid], heightMaxs[pid], SCconNodesMTP, VidtoTNidP,
                       updatedSC, true);
...
//for PCH
void Graph::IncreasePartiBatchUpdateCheckCH(int pid, vector<pair<pair<int, int>, pair<int, int>>> &wBatch,
                                            vector<pair<pair<int, int>, pair<int, int>>> &weightOverlay, bool ifOpt,
                                            vector<pair<pair<int, int>, int>> &updatedSC) {
    //partition batch Increase update
//    vector<pair<pair<int,int>,int>> updatedSC;
    ...
    if (ifOpt) {
        IncreasePartiBatchForOpt(pid, wBatch, NeighborsParti, Trees[pid], ranks[pid], heightMaxs[pid], SCconNodesMTP,
                                 VidtoTNidP, updatedSC, false);
    } else {
        IncreasePartiBatch(pid, wBatch, NeighborsParti, Trees[pid], ranks[pid], heightMaxs[pid], SCconNodesMTP,
                           VidtoTNidP, updatedSC, false);
    }
...
void
Graph::IncreasePartiBatchUpdateCheckCHV(vector<int> p, map<int, vector<pair<pair<int, int>, pair<int, int>>>> &wBatch,
                                        vector<pair<pair<int, int>, pair<int, int>>> &overlayBatch, bool ifOpt,
                                        vector<vector<pair<pair<int, int>, int>>> &updatedSCs) {
    for (int i = 0; i < p.size(); ++i) {
        int pid = p[i];
...
void Graph::IncreasePartiBatchUpdateCheckPostMHL(map<int, vector<pair<pair<int, int>, pair<int, int>>>> &partiBatch,
                                                 vector<pair<pair<int, int>, pair<int, int>>> &overlayBatch,
                                                 bool ifParallel) {
    //partition batch Increase update
    ...
    if (ifParallel) {
        if (!partiBatch.empty()) {
            if (partiBatch.size() > threadnum) {
                ...
                boost::thread_group thread;
                for (auto j = 0; j < processID.size(); ++j) {
                    thread.add_thread(new boost::thread(&Graph::IncreasePartiBatchPostMHLShortcutV, this, processID[j],
                                                        boost::ref(partiBatch), boost::ref(Tree), boost::ref(rank),
                                                        heightMax, boost::ref(updatedSCs),
                                                        boost::ref(PropagateOverlay)));
                }
                thread.join_all();
...
//            cout<<"Update Partition number: "<<partiBatch.size()<<endl;
                ...
                for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
                    int pid = it->first;
                    thread.add_thread(new boost::thread(&Graph::IncreasePartiBatchPostMHLShortcut, this, pid,
                                                        boost::ref(it->second), boost::ref(Tree), boost::ref(rank),
                                                        heightMax, boost::ref(updatedSCs[pid]),
                                                        boost::ref(PropagateOverlay)));
                }
        ...
        for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
            int pid = it->first;
//            cout<<"Shortcut update of partition "<<pid<<endl;
            IncreasePartiBatchPostMHLShortcut(pid, it->second, Tree, rank, heightMax, updatedSCs[pid],
                                              PropagateOverlay);
        }
    }
...
//            sm->notify();
                        ...
                        cout << "Something wrong happens. " << bid1 << " " << bid2 << " " << olddis << " " << newdis
                             << endl;
                        exit(1);
                    }
                } else {
...
void Graph::MixVPLBatchShortcutUpdateParti(map<int, vector<pair<pair<int, int>, pair<int, int>>>> &partiBatchDec,
                                           map<int, vector<pair<pair<int, int>, pair<int, int>>>> &partiBatchInc,
                                           vector<pair<pair<int, int>, pair<int, int>>> &overlayBatchDec,
                                           vector<pair<pair<int, int>, pair<int, int>>> &overlayBatchInc,
                                           bool ifParallel) {
...
//        if(false){
        if (ifParallel & partiBatch.size() > 1) {
            if (partiBatch.size() > threadnum) {
                ...
                for (auto j = 0; j < processID.size(); ++j) {
                    thread.add_thread(
                            new boost::thread(&Graph::MixPartiBatchVPLShortcutV, this, boost::ref(processID[j]),
                                              boost::ref(partiBatch), boost::ref(Tree), boost::ref(rank), heightMax,
                                              boost::ref(updatedSCsDec), boost::ref(updatedSCsInc)));
                }
                thread.join_all();
...
//            cout<<"Update Partition number: "<<partiBatch.size()<<endl;
                ...
                for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
                    int pid = it->first;
                    thread.add_thread(
                            new boost::thread(&Graph::MixPartiBatchVPLShortcut, this, pid, boost::ref(it->second.first),
                                              boost::ref(it->second.second), boost::ref(Tree), boost::ref(rank),
                                              heightMax, boost::ref(updatedSCsDec[pid]),
                                              boost::ref(updatedSCsInc[pid])));
                }
                thread.join_all();
            }
        } else {
//            cout<<"Single thread computation."<<endl;
            for (auto it = partiBatch.begin(); it != partiBatch.end(); ++it) {
                int pid = it->first;
//            cout<<"Shortcut update of partition "<<pid<<endl;
                MixPartiBatchVPLShortcut(pid, it->second.first, it->second.second, Tree, rank, heightMax,
                                         updatedSCsDec[pid], updatedSCsInc[pid]);
            }
        }
...
//    cout<<"updatedSC size: "<<updatedSC.size()<<endl;
        ...
        for (auto it = overlayBatchDec.begin(); it != overlayBatchDec.end(); ++it) {
            ...
            if (updatedSCTrueDec.find(make_pair(bid1, bid2)) == updatedSCTrueDec.end()) {//if not found
                ...
                cout << "Wrong. " << bid1 << " " << bid2 << ": " << updatedSCTrueDec[make_pair(bid1, bid2)] << " "
                     << newdis << endl;
                exit(1);
            }
            updateSCSize++;
...
//        cout<<"updatedSCTrueDec size: "<<updatedSCTrueDec.size()<<" "<<updateSCSize<< endl;
        ...
        for (auto it = overlayBatchInc.begin(); it != overlayBatchInc.end(); ++it) {
            ...
            if (updatedSCTrueInc.find(make_pair(bid1, bid2)) == updatedSCTrueInc.end()) {//if not found
                ...
                cout << "Wrong. " << bid1 << " " << bid2 << ": " << updatedSCTrueInc[make_pair(bid1, bid2)].second
                     << " " << newdis << endl;
                exit(1);
            }
            updateSCSize++;
...
//                    updatedSCTrueInc[make_pair(bid1, bid2)].first = wsum;
                    if (updatedSCTrueInc[make_pair(bid1, bid2)].first != wsum) {
                        cout << "1. Inconsistent wsum! " << bid1 << "(" << PartiTag[bid1].first << ","
                             << PartiTag[bid1].second << ") " << bid2 << "(" << PartiTag[bid2].first << ","
                             << PartiTag[bid2].second << ") " << updatedSCTrueInc[make_pair(bid1, bid2)].first << " "
                             << wsum << endl;
                        if (updatedSCTrueInc[make_pair(bid1, bid2)].first > wsum) {
                            updatedSCTrueInc[make_pair(bid1, bid2)].first = wsum;
                        }
            ...
            for (int i = 0; i < Tree[rank[bid1]].vert.size(); i++) {
                if (Tree[rank[bid1]].vert[i].first == bid2) {
                    ...
                    if (Tree[rank[bid1]].vert[i].second.second < 1) {//need update
                        ...
                        olddis = wsum;
                        if (newdis > olddis) {
                            overlayBatchInc.emplace_back(make_pair(bid1, bid2), make_pair(olddis,
                                                                                          newdis));//weightOverlay collect the changed edges on overlay graph
                        } else if (newdis < olddis) {
...
//        }
            ...
            if (newdis <= olddis) {//cnt may change
//            cout<<bid1<<" "<<bid2<<" "<<olddis<<" "<<newdis<<endl;
                overlayBatchDec.emplace_back(make_pair(bid1, bid2), make_pair(olddis,
                                                                              newdis));//weightOverlay collect the changed edges on overlay graph
            } else if (newdis > olddis) {
                cout << "dec Something wrong happens. " << bid1 << " " << bid2 << " " << olddis << " " << newdis
                     << endl;
...
//bottom-up shortcut update for post-boundary partition index
void Graph::IncreasePartiBatchUpdateCheckVPL(map<int, vector<pair<pair<int, int>, pair<int, int>>>> &partiBatch,
                                             vector<pair<pair<int, int>, pair<int, int>>> &overlayBatch,
                                             bool ifParallel) {
    //partition batch Increase update
    if (!partiBatch.empty()) {
...
//        cout<<"zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz"<<endl;
        ...
        IncreaseParti(a, b, oldW, newW, NeighborsParti, Trees[pid1], ranks[pid1], heightMaxs[pid1], SCconNodesMTP,
                      VidtoTNidP);
