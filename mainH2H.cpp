...
//    runtimes=100;
    ...
    if(argc > 1) {
        ...
        if(argc > 7){
            cout << "argv[7] (Cache List Number): " << argv[7] << endl;//batch size
            cacheSize = stoi(argv[7]);
        }
        if(argc > 8){
...
//    if(dataset=="beijing" || dataset=="Guangdong"){//real-life updates
        g.RealUpdateThroughputTestQueueModel(sourcePath+dataset+"_20160105_"+to_string(batchInterval)+".batchUpdates",batchNum,T_r,workerNum);
//    }else{
