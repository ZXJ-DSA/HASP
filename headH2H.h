...
//    vector<omp_lock_t> oml;
    ...
    unsigned long long
    EffiCheckThroughput(vector<pair<int, int>> &ODpair, int runtimes, int batchInterval, double &updateT,
                        double &queryT);
    ...
    void RealUpdateThroughputTestQueueModel(string updateFile, int batchNum, double T_r, int workerNum);
    ...
    pair<double, double> simulator_UpdateFirst(vector<vector<double>> &query_cost, vector<vector<double>> &update_cost,
