...
vector<int> _DD2_;//true degree, temporal degree ,_DD2_
...
void Graph::IndexConstruction(int algo) {
    ...
    switch (algoIndex) {
        ...
        default: {
            cout << "Wrong SP index! " << algoIndex << endl;
            exit(1);
        }
    }
}
...
//                cout<<"count "<<count<<" , treewidth "<<Twidth<<endl;
            ...
            for (auto it = E[x].begin(); it != E[x].end(); it++) {
                if (exist[(*it).first]) {
                    Neigh.push_back(*it);
                    if (treeHeight2[it->first] < treeHeight2[x] + 1) {//obtain the estimated tree height
                        treeHeight2[it->first] = treeHeight2[x] + 1;
                    }
                    if (heightTemp < treeHeight2[it->first]) {
//                        cout<<it->first<<" , "<<x<<" : "<<heightTemp<<" "<<treeHeight2[it->first]<<endl;
                        heightTemp = treeHeight2[it->first];
                    }
                }
            }
        ...
        SCconNodesMT.assign(node_num,
                            mi);//record the supportive vertices of a shortcut, only record edge once by leveraging the ID positions of endpoints
                    ...
                    if (Neigh.size() > threadnum) {
                        ...
                        for (int i = 0; i < threadnum; i++) {
                            ...
                            if (i == threadnum - 1)
                                ...
                                p.second = (i + 1) * step;
                            thread.add_thread(
                                    new boost::thread(&Graph::NeighborComorder, this, boost::ref(Neigh), p, x));
                        }
                        thread.join_all();
                    } else {
                        boost::thread_group thread;
...
//        cout<<s<<"("<<CoreTag[s]<<") "<<t<<"("<<CoreTag[t]<<") "<<d2<<" "<<d1<<endl;
        if (d1 != d2) {
            cout << "InCorrect! " << i << ": " << s << "(" << NodeOrder[s] << ") " << t << "(" << NodeOrder[t] << ") "
                 << d2 << " " << d1 << endl;
            exit(1);
        }
    }
...
//    cout<<endl;
        ...
        cout << "Average Query Time: " << 1000 * runT / runtimes << " ms. "
             << 1000 * (double) (clock() - start) / (CLOCKS_PER_SEC * runtimes) << " ms." << endl;
    }
}
...
//function for efficiency test
unsigned long long
Graph::EffiCheckThroughput(vector<pair<int, int>> &ODpair, int runtimes, int batchInterval, double &updateT,
                           double &queryT) {
    int s, t;
    double runT = 0;
    int d1, d2;
...
//                        g2.CorrectnessCheckH2H(100);
                ...
                cout << "\nOverall throughput: " << throughputNum << " ; Average throughput: "
                     << throughputNum / batchNum << " ; Average Decrease batch update Time: " << runT1 / batchNum
                     << " s; Average query time: " << 1000 * queryT / batchNum << " ms." << endl;
            }
            break;
        }
        case 2: {
...
//                    throughputNum += EffiCheckThroughput(ODpair,tRecord,batchInterval);//query efficiency test
                ...
                cout << "\nOverall throughput: " << throughputNum << " ; Average throughput: "
                     << throughputNum / batchNum << " ; Average Increase batch update Time: " << runT2 / batchNum
                     << " s; Average query time: " << 1000 * queryT / batchNum << " ms." << endl;
            }
...
//    ifDebug=true;
    ...
    double queryTime = EffiStageCheck(ODpair, ODpair.size(), stageQueryT);
    double updateTime = 0;
    int gap = 3600 / batchInterval;
    ...
    for (int p = 0; p < sampleBatch.size(); ++p) {
//    for(int i=0;i<batchNum;++i){
//        if(i%gap>1){
//            continue;
//        }
        int batch_i = sampleBatch[p];
        wBatchDec.clear();
        wBatchInc.clear();
        map<pair<int, int>, int> uEdges;
        for (int j = 0; j < batchUpdates[batch_i].size(); ++j) {
            ID1 = batchUpdates[batch_i][j].first.first, ID2 = batchUpdates[batch_i][j].first.second, weight = batchUpdates[batch_i][j].second;
            bool ifFind = false;
            if (ID1 > ID2) {
                int temp = ID1;
                ID1 = ID2, ID2 = temp;
...
//                exit(1);
        ...
        cout << "Batch " << batch_i << " . Decrease update number: " << wBatchDec.size()
...
//            cout<<"Batch "<<i<<". Update time: "<<tt.GetRuntime()<<" s."<<endl;
        ...
        runT += updateTime;
        cout << "Batch " << batch_i << " . Update time: " << updateTime << " s." << endl;
...
//                stageQueryT[j].clear();//new
        ...
        vector<vector<double>> stageQueryThis(2, vector<double>());//(stage, query times)
        EffiStageCheck(ODpairThis, ODpairThis.size(), stageQueryThis);
        for (int j = 0; j < stageQueryThis.size(); ++j) {
            if (duration[j] > 0) {
                fastQT = stageQueryThis[j][0];
            }
...
//            cout<<"Estimated throughput: "<<resultE.first<<" ; fastest available query time: "<<resultE.second*1000<<" ("<<fastQT<<") ms"<<endl;
        throughputNum = ThroughputSimulate(stageQueryThis, stageUpdateT, batchInterval, T_r, batchInterval, fastQT,
...
//    cout<<"\nPartiNum: "<<partiNum<<". Throughput: "<<throughputNum<<" ; Average batch update Time: "<<runT/batchNum<<" s."<<endl;
    double aveThr = 0;
    ...
    cout << "\nPartiNum: " << partiNum << ". Average Throughput: " << aveThr / throughputNums.size()
         << " ; Average batch update Time: " << runT / count << " s" << endl;
...
//            cout<<"Batch "<<i<<". Update time: "<<tt.GetRuntime()<<" s."<<endl;
    ...
    cout << "\nOverall throughput: " << throughputNum << " ; Average throughput: " << throughputNum / batchNum
         << " ; Average batch update Time: " << runT2 / batchNum << " s; Average query time: "
         << 1000 * queryT / batchNum << " ms." << endl;
...
void Graph::RandomUpdateThroughputTestQueueModel(int batchNum, int batchSize, int batchInterval, double T_r,
                                                 int workerNum) {//T_r is in second
    bool ifDebug = false;
//    ifDebug=true;
    int runtimes = 10000;
//    runtimes=1000;
...
//function for efficiency test
double Graph::EffiStageCheck(vector<pair<int, int>> &ODpair, int runtimes,
    ...
    vector<int> results(runtimes, -1);
    int cNum = min(runtimes, 1000);
    if (cNum > ODpair.size()) cNum = ODpair.size();
    /// For BiDijkstra
...
double Graph::analytical_update_first(double T_q, double T_u, double T_r, double T,
                                      double V_q) {//T is duration, T_u is the average update time for one update, T_r is in seconds, return throughput
...
double Graph::ThroughputSimulate(vector<vector<double>> &query_costs, vector<vector<double>> &update_costs,
                                 int simulation_time, double threshold_time, double period_time,
                                 double queryTime) {//period_time is update interval time, simulation_time is the overall time for simulation
    Timer tt;
    tt.start();
...
void Graph::IndexMaintenanceCHWP(int updateType, int updateSize, bool ifBatch, int batchSize) {
    ...
    cout << "Update size: " << updateSize << ". Update batch: " << updateBatch << " ; Batch size: " << batchSize
         << endl;
    vector<pair<pair<int, int>, int>> updateData;
    vector<pair<pair<int, int>, pair<int, int>>> wBatch;
    ReadUpdate(file, updateData);
...
void Graph::CHdecBat(vector<pair<pair<int, int>, pair<int, int>>> &wBatch) {
    ...
    while (!OC.empty()) {
        ...
        for (int i = 0; i < InMLower.size(); i++) {
            inID = InMLower[i].first;
            inW = InMLower[i].second;
...
void Graph::CHincBatMT(vector<pair<pair<int, int>, pair<int, int>>> &wBatch) {
    ...
    while (!OC.empty()) {
        ...
        int inID, inW;
        map<int, int> HigherIn;
        vector<pair<int, int>> LowerIn;
        ...
        for (int i = 0; i < LowerIn.size(); i++) {
            inID = LowerIn[i].first;
            inW = LowerIn[i].second;
            for (int j = 0; j < NeighborCon[inID].size(); j++) {
                if (NeighborCon[inID][j].first == t) {
...
void Graph::eachNodeProcessIncrease1(int children, vector<int> &line, int &changelabel) {
    ...
    for (int i = 0; i < Tree[children].dis.size(); i++) {
        if (Tree[children].cnt[i] <=
            0) {//if the distance label to i-th ancestor should be maintained, use <= since cnt may not be accurate after update
//        if(true){
            changelabel += 1;
