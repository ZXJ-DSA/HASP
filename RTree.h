...
public:
  ...
  /// \param a_res return the affected MBR
  /// \return Returns the number of entries found
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool (const DATATYPE&)> callback, std::vector<int>& res) const;
...
protected:
  ...
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool (const DATATYPE&)> callback, std::vector<int>& res) const;
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool (const DATATYPE&)> callback, std::vector<int>& res) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  int foundCount = 0;
  Search(m_root, &rect, foundCount, callback, res);
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool (const DATATYPE&)> callback, std::vector<int>& res) const
{
  RTREE_ASSERT(a_node);
  RTREE_ASSERT(a_node->m_level >= 0);
  RTREE_ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))// if overlap
      {
        if(!Search(a_node->m_branch[index].m_child, a_rect, a_foundCount, callback, res))// if no need to continue
        {
...
struct Rect {
    Rect(int a_minX, int a_minY, int a_maxX, int a_maxY){//: id(Rect::max_id++)
        min[0] = a_minX;
        min[1] = a_minY;
    ...
    Rect(int idd, int a_minX, int a_minY, int a_maxX, int a_maxY){//: id(Rect::max_id++)
        id = idd;
        min[0] = a_minX;
        min[1] = a_minY;
        max[0] = a_maxX;
