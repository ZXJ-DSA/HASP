...
//        weight=1;
        if (ID1 >= 0 && ID1 < node_num && ID2 >= 0 && ID2 < node_num && weight > 0) {
            if (NeighborMapT[ID1].find(ID2) == NeighborMapT[ID1].end()) {//if not found
                ...
                if (NeighborMapT[ID1][ID2] != weight) {
                    cout << "Inconsistent graph data!!! " << ID1 << " " << ID2 << " " << NeighborMapT[ID1][ID2] << " "
                         << weight << endl;
                    exit(1);
                }
            }
            ...
            cout << "Wrong data! " << ID1 << " " << ID2 << " " << weight << endl;
            exit(1);
        }
    ...
    tt.stop();
    cout << "Finish Reading! Vertex number: " << node_num << "; Edge number: " << edge_num << ". Time: "
         << tt.GetRuntime() << " s." << endl;
    if (vertices.size() != node_num) {
        cout << "vertices is wrong! " << vertices.size() << " " << node_num << endl;
        exit(1);
    }
    set<int> mcc;
...
void Graph::ReadCoordinate(string filename) {
    ...
    while (!line.empty()) {
        ...
        if (ID >= 0 && ID < node_num) {
            ...
            cout << "Wrong data! " << ID << " " << lon << " " << lat << endl;
            exit(1);
        }
...
void Graph::WritePostMHLPartiResult(string filename) {
    ...
    OF.close();
    if (vNum != node_num) {
        cout << "Inconsistent vertex number! " << vNum << " " << node_num << endl;
        exit(1);
    }
...
void Graph::getThroughputEvolveData(string filename) {
    vector<vector<pair<pair<int, int>, int>>> batchUpdates;
    int batchInterval = ReadRealUpdates(sourcePath + "/" + dataset + "_20160105_300.batchUpdates", batchUpdates);

    for (int i = 0; i < batchUpdates.size(); ++i) {
        cout << batchUpdates[i].size() << "\t";
    }
    cout << endl;
    exit(0);
    ...
    string line;
    vector<string> vs;
    vector<pair<int, vector<double>>> res;
    ...
    while (getline(inFile, line)) {
        ...
        if (vs[0] == "argv[3]") {
            if (algo != -1) {
                res.emplace_back(algo, tempRes);
                tempRes.clear();
            }
            algo = stoi(vs[3]);
        } else if (vs[0] == "Throughput:") {
            tempRes.emplace_back(stod(vs[1]));
        }
    }
    res.emplace_back(algo, tempRes);
    tempRes.clear();
...
//        cout<<PartiTag[it->ID].first<<": "<<it->ID<<" "<<NodeOrder[it->ID]<<" "<<NeighborsOverlay[it->ID].size()<<endl;
    ...
    for (int i = 0; i < boundaryNum; ++i) {
        ...
        if (!PartiTag[ID1].second) {
            cout << boundaryNum << ", " << node_num - i << ": " << ID1 << "(" << NodeOrder[ID1]
    ...
    cout << "Overall boundary vertex number: " << boundaryNum << "(" << OverlayVertex.size()
         << ") ; Average boundary vertex number for each partition: " << boundaryNum / partiNum
         << " ; Maximum boundary number: " << *max_element(bNums.begin(), bNums.end()) << endl;
    ...
    for (int i = 0; i < Neighbor.size(); i++) {
        ...
        if (IDMap[i] == -1) {
            cout << "Wrong ID map for " << i << " " << IDMap[i] << endl;
            exit(1);
        }
    }
    if (edge_num == 0) {
        ...
        if (edge_num != calEdgeNum) {
            cout << "Inconsistent edge number! " << edge_num << " " << calEdgeNum << endl;
            exit(1);
        }
    }
...
void Graph::VPLGraphPartitionRead(string filename) {
    //read the partitioned graphs
//    NeighborMap.assign(node_num,unordered_map<vertex,int>());
    Neighbor.assign(node_num, vector<pair<vertex, int>>());
    NeighborsParti.assign(node_num, vector<pair<vertex, int>>());
...
//    NeighborSketch.assign(partiNum,vector<int>());
    NeighborSketchB.assign(partiNum, unordered_map<int, vector<int>>());
    vector<set<OrderCompMax>> orderPartiV(partiNum, set<OrderCompMax>());//higher-order first
...
//            if(NeighborSketchS[PartiTag[ID1].first].find(PartiTag[ID2].first)==NeighborSketchS[PartiTag[ID1].first].end()){//if not found
//                NeighborSketchS[PartiTag[ID1].first].insert(PartiTag[ID2].first);
//                NeighborSketchS[PartiTag[ID2].first].insert(PartiTag[ID1].first);
//            }
            if (NeighborSketchB[PartiTag[ID1].first].find(PartiTag[ID2].first) ==
                NeighborSketchB[PartiTag[ID1].first].end()) {//if not found
                NeighborSketchB[PartiTag[ID1].first].insert({PartiTag[ID2].first, vector<int>()});
                NeighborSketchB[PartiTag[ID2].first].insert({PartiTag[ID1].first, vector<int>()});
                NeighborSketchB[PartiTag[ID1].first][PartiTag[ID2].first].push_back(ID2);
                NeighborSketchB[PartiTag[ID2].first][PartiTag[ID1].first].push_back(ID1);
            } else {
                NeighborSketchB[PartiTag[ID1].first][PartiTag[ID2].first].push_back(ID2);
                NeighborSketchB[PartiTag[ID2].first][PartiTag[ID1].first].push_back(ID1);
            }
...
//            }
    ...
    int neiNum = 0;
    for (int i = 0; i < NeighborSketchB.size(); ++i) {
        if (!NeighborSketchB[i].empty()) {
            neiNum += NeighborSketchB[i].size();
        }
    }
    cout << "Average neighbor partition number: " << (double) neiNum / partiNum << endl;
...
//        cout<<PartiTag[it->ID].first<<": "<<it->ID<<" "<<NodeOrder[it->ID]<<" "<<NeighborsOverlay[it->ID].size()<<endl;
    ...
    for (int pid = 0; pid < partiNum; ++pid) {
        ...
        for (auto it = orderPartiV[pid].begin(); it != orderPartiV[pid].end(); ++it) {
//            cout<<pid<<": "<<it->ID<<" "<<NodeOrder[it->ID]<<" "<<PartiTag[it->ID].second<<endl;
            if (PartiTag[it->ID].second) {
                BoundVertex[pid].push_back(it->ID);
                BoundVertexMap[pid].insert({it->ID, j});
                j++;
            } else {
                break;
            }
...
//        OF<<" "<<Tree[ID1].treeroot;
        ...
        for (int i = 0; i < Tree[ID1].vert.size(); ++i) {
            OF << " " << Tree[ID1].vert[i].first << " " << Tree[ID1].vert[i].second.first << " "
               << Tree[ID1].vert[i].second.second << " " << Tree[ID1].pos[i];
        }
        OF << " " << Tree[ID1].vAncestor.size() - 1;//dis
        for (int i = 0; i < Tree[ID1].vAncestor.size() - 1; ++i) {
            OF << " " << Tree[ID1].vAncestor[i] << " " << Tree[ID1].dis[i] << " " << Tree[ID1].cnt[i] << " "
               << Tree[ID1].FN[i];
        }
...
int Graph::ReadRealUpdates(string filename, vector<vector<pair<pair<int, int>, int>>> &batchUpdates) {
    ...
    int batchSize;
    batchUpdates.assign(batchNumMax, vector<pair<pair<int, int>, int>>());
    ...
    cout << "Update batch number: " << batchNumMax << " ; Average batch size: " << aveBatchSize
         << " ; Maximal batch size: " << maxBatchSize << " ; Minimal batch size: " << minBatchSize
         << " ; Batch interval: " << batchInterval << endl;
    return batchInterval;
}

void Graph::GetUpdatePortion2(int batchInterval) {
    vector<vector<pair<pair<int, int>, int>>> batchUpdates;

    for (int day = 1; day <= 5; ++day) {
        string filename = sourcePath + "/" + dataset + "_2016010" + to_string(day) + "_" + to_string(batchInterval) +
                          ".batchUpdates";
        int batchInterval = ReadRealUpdates(filename, batchUpdates);
        ...
        for (int i = 0; i < batchUpdates.size(); ++i) {
            ...
            for (int j = 0; j < batchUpdates[i].size(); ++j) {
                ID1 = batchUpdates[i][j].first.first, ID2 = batchUpdates[i][j].first.second, newW = batchUpdates[i][j].second;
                bool ifFind = false;
                if (ID1 > ID2) {
                    int temp = ID1;
                    ID1 = ID2, ID2 = temp;
                ...
                for (int k = 0; k < Neighbor[ID1].size(); ++k) {
                    if (Neighbor[ID1][k].first == ID2) {
                        ifFind = true;
                        oldW = Neighbor[ID1][k].second;
                        Neighbor[ID1][k].second = newW;
                        if (oldW > newW) {
                ...
                for (int k = 0; k < Neighbor[ID2].size(); ++k) {
                    if (Neighbor[ID1][k].first == ID1) {
                        Neighbor[ID1][k].second = newW;
                    }
                }
            ...
            cout << "Batch " << i << ". Decrease update number: " << wBatchDec.size() << " ; Increase update number: "
                 << wBatchInc.size() << endl;
            DecNums.emplace_back(wBatchDec.size());
            IncNums.emplace_back(wBatchInc.size());
        }

        cout << "Writing batch size info into disk..." << endl;
        string file = sourcePath + "/" + dataset + "_2016010" + to_string(day) + "_" + to_string(batchInterval) +
                      ".batchUpdatesPortion2";
        ofstream OF(file);
        if (!OF) {
            cout << "Cannot open " << file << endl;
            exit(1);
        }
        OF << IncNums.size() << endl;
        for (int i = 0; i < IncNums.size(); ++i) {
            OF << i << " " << DecNums[i] << " " << IncNums[i] << endl;
        }
        OF.close();
    }
...
void Graph::GetUpdatePortion(string filename) {
    vector<vector<pair<pair<int, int>, int>>> batchUpdates;
    ...
    cout << "Writing batch size info into disk..." << endl;
    string file = sourcePath + "/" + dataset + "_20160105_" + to_string(batchInterval) + ".batchUpdatesPortion";
    ofstream OF(file);
    if (!OF) {
        cout << "Cannot open " << file << endl;
        exit(1);
...
void Graph::GetQueryDistribution(string filename, int batchInterval) {
    vector<vector<pair<pair<int, int>, pair<int, int>>>> ODpairs;
    string fileQuery;
    fileQuery = sourcePath + "/" + dataset + ".realQueries";
    ifstream IF(fileQuery);
    if (!IF) {
        cout << "Cannot open file " << filename << endl;
        exit(1);
    }
    cout << "Query file: " << filename << endl;
    int num, ID1, ID2, times, dis, day, time;
    unsigned long long timestamp;
    ODpairs.assign(5, vector<pair<pair<int, int>, pair<int, int>>>());
    IF >> num;
    int maxDis = 0, minDis = 9999999;
    int maxDis2 = 0, minDis2 = 9999999;
    for (int k = 0; k < num; k++) {
        IF >> timestamp >> ID1 >> ID2 >> times >> dis;
        int tempT = timestamp - 1451577600;
        day = tempT / 86400;
        if (day == 4) {
            maxDis = max(maxDis, dis);
            minDis = min(minDis, dis);
        }
        maxDis2 = max(maxDis2, dis);
        minDis2 = min(minDis2, dis);
        for (int i = 0; i < times; ++i) {
            ODpairs[day].emplace_back(make_pair(ID1, ID2), make_pair(tempT, dis));
        }

    }
    IF.close();
    int setNum = 10;
    double gapPower = setNum;
    gapPower = 1 / gapPower;

    double gap = pow(maxDis / minDis, gapPower);
    gap = (maxDis - minDis) / setNum;
    cout << "Query number: " << num << " ; maxDis: " << maxDis2 << " ; minDis: " << minDis2 << " ; maxDis of day 5: "
         << maxDis << " ; minDis of day 5: " << minDis << " ; gap: " << gap << endl;

    vector<vector<pair<int, int>>> QDis(setNum, vector<pair<int, int>>());
    vector<pair<int, int>> queryPortion(setNum, make_pair(0, 0));//same-parti, cross-parti
    vector<pair<double, double>> ranges;
    for (int i = 0; i < setNum; ++i) {
//        ranges.emplace_back(minDis*pow(gap,i),minDis*pow(gap,i+1));
        ranges.emplace_back(minDis + gap * i, minDis + gap * (i + 1));
    }
    ranges[setNum - 1].second = maxDis + 1;
    int batchNum = 86400 / batchInterval;
    vector<int> batchQNum(batchNum, 0);
    cout << "Batch interval: " << batchInterval << " ; batch number: " << batchNum << endl;
    for (int i = 0; i < ODpairs[4].size(); ++i) {
        ID1 = ODpairs[4][i].first.first, ID2 = ODpairs[4][i].first.second, times = ODpairs[4][i].second.first, dis = ODpairs[4][i].second.second;

        int d_i = 0;
        while (true) {
            if (dis >= ranges[d_i].first && dis < ranges[d_i].second) {
                break;
            } else {
                d_i++;
                if (d_i >= setNum) {
                    cout << "Wrong. " << d_i << " " << dis << " " << ranges[9].second << endl;
                    exit(1);
                }
            }
        }
        if (PartiTag[ID1].first == PartiTag[ID2].first) {
            queryPortion[d_i].first++;
        } else {
            queryPortion[d_i].second++;
        }
        QDis[d_i].emplace_back(ID1, ID2);

        d_i = (times - 345600) / batchInterval;
//        cout<<times<<" "<<d_i<<endl;
        batchQNum[d_i]++;
    }
    cout << "Writing query info into disk..." << endl;
    string file = sourcePath + "/" + dataset + "_20160105.realQueriesDistribute";
    ofstream OF(file);
    if (!OF) {
        cout << "Cannot open " << file << endl;
        exit(1);
    }
    OF << ODpairs[4].size() << " " << setNum << " " << minDis << " " << maxDis << endl;

    for (int i = 0; i < QDis.size(); ++i) {
        OF << i << " " << ranges[i].first << " " << ranges[i].second << " " << QDis[i].size() << " "
           << queryPortion[i].first << " " << queryPortion[i].second << endl;
        cout << i << " " << ranges[i].first << " " << ranges[i].second << " " << QDis[i].size() << " "
             << queryPortion[i].first << " " << queryPortion[i].second << endl;
    }
//    int temp_i=QDis.size()-1;
//    OF<<temp_i<<" "<<minDis+gap*temp_i<<" "<<maxDis<<" "<<QDis[temp_i].size()<<endl;
//    cout<<temp_i<<" "<<minDis+gap*temp_i<<" "<<maxDis<<" "<<QDis[temp_i].size()<<endl;
    OF.close();
    string file2 = sourcePath + "/" + dataset + "_20160105.realQueriesInfo";
    ofstream OF2(file2);
    if (!OF2) {
        cout << "Cannot open " << file2 << endl;
        exit(1);
    }
    OF2 << batchQNum.size() << " " << batchInterval << endl;

    for (int i = 0; i < batchQNum.size(); ++i) {
        OF2 << i << " " << batchQNum[i] << endl;
//        cout<<i<<" "<<batchQNum[i]<<endl;
    }
//    int temp_i=QDis.size()-1;
//    OF<<temp_i<<" "<<minDis+gap*temp_i<<" "<<maxDis<<" "<<QDis[temp_i].size()<<endl;
//    cout<<temp_i<<" "<<minDis+gap*temp_i<<" "<<maxDis<<" "<<QDis[temp_i].size()<<endl;
...
vector<int> Graph::SampleBatches(string filename, int batchNum) {
    ifstream IF(filename);
    if (!IF) {
        cout << "Cannot open file " << filename << endl;
        exit(1);
    }
    cout << "Update info file: " << filename << endl;
    int num, ID, value;
    vector<int> batches;
    set<int> batchResultSet;

    IF >> num;
    cout << "Total batch number: " << num << " ; sample batch number: " << batchNum << endl;
    double aveV = 0;
    pair<int, int> maxPair, minPair;
    maxPair.first = 0, minPair.first = 99999999;
    for (int k = 0; k < num; k++) {
        IF >> ID >> value;
        batches.emplace_back(value);
        aveV += value;
        if (maxPair.first < value) {
            maxPair.first = value, maxPair.second = ID;
        }
        if (minPair.first > value) {
            minPair.first = value, minPair.second = ID;
        }
    }
    IF.close();
    int range = maxPair.first - minPair.first;
    cout << "Average batch number: " << aveV / num << " ; max batch number: " << maxPair.first << " (" << maxPair.second
         << ") ; min batch number: " << minPair.first << " (" << minPair.second << ") ; range: "
         << maxPair.first - minPair.first << endl;
    batchResultSet.insert(maxPair.second);
    batchResultSet.insert(minPair.second);


    int sumSize = batchNum * aveV;
    sumSize = sumSize - maxPair.first - minPair.first;
    if (maxPair.second < num - 1 && sumSize > batches[maxPair.second + 1]) {
        batchResultSet.insert(maxPair.second + 1);
        sumSize -= batches[maxPair.second + 1];
    }
    if (minPair.second < num - 1 && sumSize > minPair.first) {
        batchResultSet.insert(minPair.second + 1);
        sumSize -= batches[minPair.second + 1];
    }

    int bLeft = batchNum - batchResultSet.size();
    int gap = num / bLeft;
    ...
    for (int i = 0; i < bLeft; ++i) {
        ...
        bool flag = false;
        while (id < num) {
            if (batchResultSet.find(id) == batchResultSet.end()) {//if not found
                batchResultSet.insert(id);
                flag = true;
                break;
            } else {
                id++;
    ...
    vector<int> Result;//batch size, batch ID
    for (auto it = batchResultSet.begin(); it != batchResultSet.end(); ++it) {
        Result.emplace_back(*it);
    }
    return Result;
}

void Graph::ReadRealQueries(string filename, vector<vector<pair<pair<int, int>, int>>> &ODpair) {
    ifstream IF(filename);
    if (!IF) {
        cout << "Cannot open file " << filename << endl;
        exit(1);
    ...
    cout << "Query file: " << filename << endl;
    int num, ID1, ID2, times, dis, day;
    unsigned long long timestamp;
    ODpair.assign(5, vector<pair<pair<int, int>, int>>());
    IF >> num;
    ...
    for (int k = 0; k < num; k++) {
        ...
        day = tempT / 86400;
        for (int i = 0; i < times; ++i) {
            ODpair[day].emplace_back(make_pair(ID1, ID2), tempT);
        }
...
pair<int, vector<pair<int, int>>> Graph::DijkstraPath(int ID1, int ID2, vector<vector<pair<vertex, int>>> &Neighbor) {
    benchmark::heap<2, int, int> pqueue(node_num);
    pqueue.update(ID1, 0);
    ...
    RetrievePath(ID1, ID2, prece, path);
    cout << "Path length: " << d << endl;
...
int Graph::EuclideanDis(int s, int t) {
    int R = 6371;//km
    double p = PI / 180;
    double lon1 = (double) GraphLocation[s].first / 1000000;
    double lat1 = (double) GraphLocation[s].second / 1000000;
    double lon2 = (double) GraphLocation[t].first / 1000000;
    double lat2 = (double) GraphLocation[t].second / 1000000;
...
void Graph::RetrievePath(int ID1, int ID2, vector<int> &prece, vector<pair<int, int>> &pathInfo) {
    ...
    path.push_back(ID1);
    pathInfo.assign(path.size(), pair<int, int>());
    pair<int, int> highestVertex(-1, 0);//ID, order
    cout << "path from " << ID1 << " to " << ID2 << ": " << endl;
    for (int i = path.size() - 1; i > -1; i--) {
        cout << " " << path[i] << "(" << PartiTag[path[i]].first << "," << PartiTag[path[i]].second << ","
             << NodeOrder[path[i]] << ") ";//<<endl;
        ...
        if (i > 0) {
            for (int j = 0; j < Neighbor[path[i]].size(); ++j) {
                if (Neighbor[path[i]][j].first == path[i - 1]) {
                    ...
                    pathInfo[i].first = path[i], pathInfo[i].second = Neighbor[path[i]][j].second;
                    break;
                }
            }
        } else {
...
int Graph::DijkstraCore(int ID1, int ID2) {
    if (!PartiTag[ID1].second || !PartiTag[ID2].second) {
        cout << "Wrong! There is non-boundary vertex! " << ID1 << "(" << PartiTag[ID1].second << ") " << ID2 << "("
             << PartiTag[ID2].second << ")" << endl;
        return -1;
    }
    benchmark::heap<2, int, int> pqueue(node_num);
    pqueue.update(ID1, 0);
...
void Graph::UpdateGene(int num, string filename) {
    ...
    for (int k = 0; k < num; k++) {
        ...
        if (eid.find(edgeid) == eid.end()) {
            OF << ENodeID[edgeid].first.first << " " << ENodeID[edgeid].first.second << " " << ENodeID[edgeid].second
               << endl;
            eid.insert(edgeid);
        } else {
            k--;
...
//    ODGeneCrossParti(100000,sourcePath+"/"+dataset+"partitions/"+dataset+"_"+algoParti+"_"+ to_string(partiNum)+"CrossParti.query");//cross-partition simulation
        for (int i = 100; i >= 50; i -= 10) {
            double portion = i;
            portion /= 100;
            ODGeneParti(100000, sourcePath + "/partitions/" + dataset + "_" + algoParti + "_" + to_string(partiNumber) +
                                "/sameParti_" + to_string(i) + ".query", portion);
        }
        ...
        if (!IF) {//if the label file does not exist, construct it
            ...
            ReadOrder(orderfile);
            ReadPostMHLPartiResult(sourcePath + "/tmp/" + dataset + ".PostMHL_" + to_string(bandWidth) +
                                   ".partiInfo");//read partitions
            IF.close();
        }

        for (int i = 100; i >= 50; i -= 10) {
            ODGeneParti(100000, sourcePath + "/tmp/" + dataset + ".PostMHL_" + to_string(bandWidth) + ".sameParti_" +
                                to_string(i) + ".query", i);
        }
    }
