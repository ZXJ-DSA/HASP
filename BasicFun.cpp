...
//            }
            if(NeighborSketchB[PartiTag[ID1].first].find(PartiTag[ID2].first)==NeighborSketchB[PartiTag[ID1].first].end()){//if not found
...
int Graph::ReadRealUpdates(string filename, vector<vector<pair<pair<int,int>,int>>>& batchUpdates){
    ...
    cout<<"Update batch number: "<<batchNumMax<<" ; Average batch size: "<<aveBatchSize<<" ; Maximal batch size: "<<maxBatchSize<<" ; Minimal batch size: "<<minBatchSize<<" ; Batch interval: "<< batchInterval<<endl;
...
void Graph::GetUpdatePortion2(int batchInterval) {
    ...
    for(int day=1;day<=5;++day){
        string filename=sourcePath+"/"+dataset+"_2016010"+to_string(day)+"_"+to_string(batchInterval)+".batchUpdates";
        ...
        for(int i=0;i<batchUpdates.size();++i) {
            ...
            DecNums.emplace_back(wBatchDec.size()); IncNums.emplace_back(wBatchInc.size());
        }

        cout<<"Writing batch size info into disk..."<<endl;
        string file=sourcePath+"/"+dataset+"_2016010"+to_string(day)+"_"+to_string(batchInterval)+".batchUpdatesPortion2";
        ofstream OF(file);
        if(!OF){
...
void Graph::GetQueryDistribution(string filename, int batchInterval) {
    ...
    for(int k=0;k<num;k++){
        ...
        day=tempT/86400;
        if(day==4){
            maxDis=max(maxDis,dis); minDis=min(minDis,dis);
        }
        maxDis2=max(maxDis2,dis); minDis2=min(minDis2,dis);
    ...
    cout<<"Query number: "<<num<<" ; maxDis: "<<maxDis2<<" ; minDis: "<<minDis2<<" ; maxDis of day 5: "<<maxDis<<" ; minDis of day 5: "<<minDis<<" ; gap: "<<gap<<endl;
    ...
    for(int i=0;i<ODpairs[4].size();++i) {
        ...
        while(true){
            if(dis>=ranges[d_i].first && dis<ranges[d_i].second){
                ...
                d_i++;
                if(d_i>=setNum){
                    cout<<"Wrong. "<<d_i<<" "<<dis<<" "<<ranges[9].second<<endl; exit(1);
                }
            }
        }
        if(PartiTag[ID1].first==PartiTag[ID2].first){
...
//        cout<<times<<" "<<d_i<<endl;
    ...
    for(int i=0;i<QDis.size();++i){
        OF<<i<<" "<<ranges[i].first<<" "<<ranges[i].second<<" "<<QDis[i].size()<<" "<< queryPortion[i].first<<" "<<queryPortion[i].second<<endl;
        cout<<i<<" "<<ranges[i].first<<" "<<ranges[i].second<<" "<<QDis[i].size()<<" "<< queryPortion[i].first<<" "<<queryPortion[i].second<<endl;
    }
...
vector<int> Graph::SampleBatches(string filename, int batchNum){
    ...
    cout<<"Average batch number: "<<aveV/num<<" ; max batch number: "<<maxPair.first<<" ("<<maxPair.second<<") ; min batch number: "<<minPair.first<<" ("<<minPair.second<<") ; range: "<<maxPair.first-minPair.first<<endl;
